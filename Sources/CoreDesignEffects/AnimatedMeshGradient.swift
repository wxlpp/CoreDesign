...
///
/// ⚠️ **上面第二条要读成「`colors` 与 `alternateColors` *都*为空 ⇒ 回落 `.tint`」**
///（#253 PR #273 终审 S-2）：只给 `alternateColors`、不给 `colors` 时，本组件**静态地**
/// 用那一组色板绘制，**不取 `.tint`**（也无从混合——只有一组）。裁决点与理由逐字记在
/// `MeshDrift.blended(base:alternate:phase:)` 上。
///
...
///
/// ⇒ 真实成本是「**可见窗口的底色在失焦时消失**」，不是「快照里少一层装饰」。
/// App 切换器快照只是这一族里最轻的一个实例。
///
...
nonisolated enum MeshDrift {
    ...
    ///
    /// ⚠️ **登记一个不对称的组合**（#253 PR #273 终审 S-2）：`base` 空、`alternate` 非空时
    /// 本函数返回 `second` ⇒ 该组合**静态地**使用备用色板，**不回落 `.tint`**、也不混合
    ///（没有第二组可混）。它与类型文档里那句无条件的「`colors` 为空 ⇒ 回落 `.tint`」
    /// 字面不符，故在此点名：那句话的完整形式是「**两组都**为空 ⇒ 回落 `.tint`」。
    /// ⚠️ **有意不归一化成 `nil`**：调用方显式给了 9 个颜色，把它们丢掉去取 `.tint`
    /// 比"用你给的那组"更让人意外。⇒ 记下来，不改行为。
    ///
