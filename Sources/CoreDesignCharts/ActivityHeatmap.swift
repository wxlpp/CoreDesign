...
public struct ActivityHeatmap<Day: HeatmapDay>: View {
    ...
    // ⚠️⚠️ **上一版在同一段里先作废了数字、又据它做决策**（第 5 轮终审 I-3）：
    // 它写「旧数 8 ms 不再描述当前代码」，三行后又说「8 ms 不值得引入缓存」
    // ——**没有替代测量**。而且成本模型也归错了：
    //
    // 评审实测（`-O`，1830 天）：`weeks` 的 `Calendar` 循环 **12–21 ms**、
    // 而 `sorted` 只有 **0.03 ms** ⇒ 主项**不是** `O(N log N)` 排序，
