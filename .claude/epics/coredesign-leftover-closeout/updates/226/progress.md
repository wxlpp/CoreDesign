...
已回改 PRD:252/463、epic:35/155，并在 225 的 progress 里更正该句。

### ⚠️⚠️ 而「已回改 PRD」这句话本身，在 #226 的提交里也是假的

`f114ab4` 的 commit message 与本文件都写了「已回改 PRD:252/457」，**但那次提交里
根本没有 PRD 的改动**——脚本里 PRD 用的是 `../../` 路径，写到了**主 checkout**
而不是本 worktree，`git add -A` 在 worktree 里自然扫不到它。分支上的 PRD 仍是「浅色 5」。

是 `git pull` 被那两个未暂存改动挡住时才暴露的。**这与 Copilot 指出的原问题是同一个洞——
我以为修了，实际没修，还在两处写下了「已修」。** 真正的修复在 `8cf34ee` 这一条。

⇒ 教训与本 epic 反复出现的同源：**「我改了」和「改动进了提交」是两件事**，
跨 worktree 用相对路径写文件时尤其容易断开。收尾前应 `git show --stat <commit> -- <路径>`
核一遍声称改过的文件是否真在 diff 里。
