...
PRD 写于 Phase 1 之前,不知道地基会塌缩。**Task 001 先把 `.content` / `.card` 改指 raised 背景**,`Card` 才封装得出一个真正可见的卡片。这是对 PRD 意图(Card 是能看见的容器)的忠实,而非对其字面(封装 `.content`)的机械执行。
...
| 004 | 系统控件 `.core` style 套件(5 个,强调色走 `.tint`) | 001 | ✅ 与 002 |
...
### 内部
- 002 / 003 / 004 全部依赖 001:容器与设置行都需要 raised 背景与画布拉开;style 套件虽不直接用 surface,但同批交付,基线一致。
- 003 依赖 002:分组内的分隔线与页眉复用 `Separator` / `SectionHeader`。
- 005 依赖 002+003+004:要一屏内同时摆出这些组件才谈得上「复刻设置页」。
- 002 与 004 无文件冲突(`Components/Card` vs `Components/Style`),可并行。
...
- [ ] 003.md - 分组设置行 InsetGroupedSection / SettingsRow (parallel: false, depends_on: 001, 002)
- [ ] 004.md - 系统控件 .core style 套件 (parallel: true, depends_on: 001)
- [ ] 005.md - 视觉终审：实机复刻一屏 iOS 设置页 (parallel: false, depends_on: 002, 003, 004)
- [ ] 006.md - 文档与发版 0.4.0 (parallel: false, depends_on: 005)

Total tasks: 6
Parallel tasks: 2（002 ∥ 004，在 001 之后）
