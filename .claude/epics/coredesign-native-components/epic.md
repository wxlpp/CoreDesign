...
github: https://github.com/wxlpp/CoreDesign/issues/139
---
...
PRD 写于 Phase 1 之前,不知道地基会塌缩。**Task #140 改 `surfaceCard` token 的别名(`surfaceCanvas` → `surfaceRaised`),`.content` / `.card` 两个 case 自动跟随**,`Card` 才封装得出一个真正可见的卡片。(评审定死走 token 别名而非改 case:可测 + 修掉公开 token 名实相悖的陷阱。)这是对 PRD 意图(Card 是能看见的容器)的忠实,而非对其字面(封装 `.content`)的机械执行。
...
与 Phase 1 不同,本 epic 基本不删不改公开符号(唯一的 token 指向修改是 ADR-1 的 `.content`/`.card`,属同名换值,记入 BREAKING-CHANGES)。但**全部交付物是视觉组件**——机械判据(编译/测试/grep)证明不了它们好不好看。Phase 1 的教训:深色语义色缺陷、320×480 兼容模式、`.preferredColorScheme` 泄漏,全是只有实机截图才发现的。本 epic 的视觉终审(Task #144)是硬门,不是可选。

## Technical Approach

### 表面色地基(`Modifier/SurfaceModifier.swift`、`Colors/SurfaceColors.swift`)

Task #140:`.content` / `.card` 的 `SurfaceKind.background` 从 `surfaceCard`(= 画布色)改指 `surfaceRaised`(= `secondarySystemGroupedBackground`)。加守卫测试:`.content` 与 `.canvas` 在深色下不同色(复用 Phase 1 的 `SurfaceContrastTests` 模式)。
...
1. **修地基**(#140)——SurfaceKind 塌缩。后面所有容器的前提。
2. **建组件**(#141 基础容器 ∥ #143 style 套件;#142 分组设置行依赖 #140+#141)——#141 与 #143 无文件冲突可并行;#142 复用 #141 的 Separator/SectionHeader。
3. **看**(#144)——视觉终审,实机复刻一屏 iOS 设置页(Success Criteria #10)。
4. **发**(#145)——BREAKING-CHANGES 0.4.0、组件索引、快照、`0.4.0` tag。

## Task Breakdown Preview

| # | 任务 | 依赖 | 并行 |
|---|---|---|---|
| #140 | 修 `SurfaceKind` 表面色塌缩(`.content`/`.card` → raised) | — | ❌ |
| #141 | 基础容器:`Card` / `Separator` / `SectionHeader` / `SectionFooter` | #140 | ✅ 与 #143 |
| #142 | 分组设置行:`InsetGroupedSection` / `SettingsRow` | #140, #141 | ❌ |
| #143 | 系统控件 `.core` style 套件(5 个,强调色走 `.tint`) | — | ✅ 从起点起 |
| #144 | 视觉终审:实机复刻一屏 iOS 设置页 + 深浅双模式 | #141, #142, #143 | ❌ |
| #145 | 文档与发版:BREAKING-CHANGES 0.4.0 / 组件索引 / 快照 / `0.4.0` tag | #144 | ❌ |
...
### 内部
- #141 依赖 #140:容器需要 raised 背景与画布拉开。**#143 不依赖 #140**(评审 S-1:style 套件不消费 surface),可从 epic 起点即与 #140 并行。
- #142 依赖 #141:分组内的分隔线与页眉复用 `Separator` / `SectionHeader`。
- #144 依赖 #141+#142+#143:要一屏内同时摆出这些组件才谈得上「复刻设置页」。
- #141 与 #143、#143 与 #140 均无文件冲突,可并行。

### 外部
- SwiftUI iOS 26 / macOS 26:`Toggle`/`TextField`/`ProgressView`/`Label`/`DisclosureGroup` 的 style 协议、`TintShapeStyle`、`.glassEffect()`。
- `ios-visual-reviewer` agent:Task #144 判定方。
...
- #140:半天,改两个 case + 守卫测试,但它是全 epic 的地基。
- #141:1 天,四个容器组件。
- #142:1.5–2 天,`SettingsRow` 的 accessory 多态 + 分隔线 leading inset 对齐是本 epic 最细的活。
- #143:2 天,5 个 style 各自的 `.tint` 接入 + 深浅态验证。
- #144:1 天,含视觉评审的修复轮次。
- #145:1 天。

合计约 7–8 天。关键路径 #140 → #141 → #142 → #144 → #145;#143 搭 #141 并行。

## Tasks Created

- [ ] #140 - 修 SurfaceKind 表面色塌缩 (parallel: false)
- [ ] #141 - 基础容器 Card / Separator / SectionHeader / SectionFooter (parallel: true, depends_on: #140)
- [ ] #142 - 分组设置行 InsetGroupedSection / SettingsRow (parallel: false, depends_on: #141)
- [ ] #143 - 系统控件 .core style 套件 (parallel: true, depends_on: none)
- [ ] #144 - 视觉终审：实机复刻一屏 iOS 设置页 (parallel: false, depends_on: #141, #142, #143)
- [ ] #145 - 文档与发版 0.4.0 (parallel: false, depends_on: #144)

Total tasks: 6
Parallel tasks: #143 从起点即可与 #140 并行；#141 ∥ #143 在 #140 之后
Sequential tasks: 4（#140 → …→ #144 → #145）
