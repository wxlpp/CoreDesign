...
github: https://github.com/wxlpp/CoreDesign/issues/116
---
...
Sources 内需改名的 `.coreFont` 调用有 42 处，另有 18 处旧 `*Font` static var（App 宿主 17 + `Avatar.swift` 1），删 `CoreRadius.mediumPlus` 直接打断 `Sidebar.swift:157,411`。若 Task #119 硬改名，`swift build` 会从 #119 一直红到迁移完成（约 36 小时），期间 CI 与每个 checkpoint 评审全部失去把关能力，且 #119∥#120 的并行声明失效——两者之一注定落在红树上无法独立验证。

因此 #119 采用 `@available(*, deprecated, renamed:)` 别名过渡：新旧名并存，全程 build 绿。**「绿」的判据必须包含 CI 的 xcodebuild iOS 腿**——`swift test` 在 macOS 上跑不到 `#if os(iOS)` 的 suite，只看它会假绿（`captionSmallDoesNotScale` 正是这样一条只在 iOS 腿上红的断言，故其翻转与 `CoreFontModifier` 的重写都必须留在 #119 内）。迁移由 deprecation warning 驱动，比编译错误清单更完整（warning 能逐点列全且不阻塞构建）。别名在 Task #121 末尾删除。

**注意别名兜不住同名换值**（见 ADR-6）——`CoreRadius.small` 3 → 6 这类改动既不报错也不产生 warning。两类风险的处置手段不同，故拆成 #121（改名，编译器驱动）与 #122（换值，人工逐点）两个任务。
...
因此 Task #122 的产出不只是「改完能编译」，而是一份「旧档位 → 新档位」的逐调用点迁移映射表，逐点确认新档位是有意选择。

### ADR-7 · 删除先于重铸

见 Overview。Task #117 / #118 无前置依赖且可并行；其余任务在其之后。
...
- `CoreRadius`：`none 0 / small 6 / medium 10 / large 16 / xLarge 22`；`smallPlus` / `mediumPlus` 先标 deprecated、Task #121 删除；新增 shape helper。
- `CoreControlMetrics`：高度 `28 / 32 / 44 / 50 / 56`；`primerVerticalPadding` 逃生口先标 deprecated、Task #121 删除。
...
1. **削面积**（#117 ∥ #118）——删掉不需要迁移的东西，把后续迁移面缩到最小。
2. **铸地基**（#119 ∥ #120）——token 定义层重写，此时不动组件；靠弃用别名保持 build 绿（ADR-5）。
3. **迁调用与收尾**（#121 → #122 → #123 → #124 → #125 → #126）——机械改名、逐点重审、可访问性验证、注释清理、视觉终审、文档发版。

风险控制：

- 第 3 段全程串行，因为这些任务会大面积触及同一批组件文件，并行只会制造冲突。
- **#121 与 #122 必须分开**：前者是编译器驱动的机械替换，后者是人工的逐点视觉判断。合在一起执行，结果通常是机械替换做完了而设计判断被跳过。
- **视觉终审（#125）排在文档发版（#126）之前**：#125 允许回改 token 值，若文档先定稿，「同名换值」表与取值理由会当场失真。#125 的评审看的是模拟器截图，不需要 `docs/snapshots/` 就绪，所以这个顺序没有代价。
- #125 设 2 轮复审上限，超出则升级给用户裁决——「处置→复审」是无界循环，且允许回卷 #119/#120 的定案。

## Task Breakdown Preview

| # | 任务 | 依赖 | 并行 |
|---|---|---|---|
| #117 | 删除 6 个 GitHub 专用组件（Sources / Tests / docs / App） | — | ✅ 与 #118 |
| #118 | 删除 Blossom trait 与 CoreGradient（Sources / Tests / CI / App 宿主配置） | — | ✅ 与 #117 |
| #119 | 重铸字体 / 圆角 / 尺寸 / 阴影 token，引入 shape helper，留弃用别名 | #118 | ✅ 与 #120 |
| #120 | 重铸语义色层：完整映射表 + macOS 降级 + accent 衍生族 + `ButtonRoleStyleRole` | #118 | ✅ 与 #119 |
| #121 | 组件机械改名扫尾并删除弃用别名（编译器驱动） | #119, #120 | ❌ |
| #122 | 同名换值逐点重审与迁移映射表（人工判断） | #121 | ❌ |
| #123 | 触控目标与 Dynamic Type 验证 | #122 | ❌ |
| #124 | 清理注释：删内部流程编号、统一中文、去 Primer 考据 | #123 | ❌ |
| #125 | 视觉终审：截图 → `ios-visual-reviewer` → 修（2 轮上限） | #124 | ❌ |
| #126 | 文档 / CI / 版本：DESIGN-FOUNDATION、BREAKING-CHANGES、CLAUDE.md、AGENTS.md、组件索引、快照、`0.3.0` 发版 | #125 | ❌ |
...
- #119 / #120 依赖 #118：`CoreGradient` 在 `Tokens/`，`#if Blossom` 分流在 `SurfaceColors` / `InteractionColors` / `ColorGrade`，先删可避免在将被删除的分支上做迁移。
- #121 依赖 #119 + #120：组件迁移需要新 token 与别名都已就位。
- #122 依赖 #121：改名扫干净、树是绿的，才能专注做视觉判断。
- #125 的实际可运行性依赖 #117 / #118 修好 `App/`（宿主 trait 配置与被删组件引用）。
- #126 依赖 #125：文档必须以视觉终审后的终态数值写就。
- #117 与 #118 无文件冲突：#117 改 `App/Sources/*.swift`，#118 改 `App/project.yml` 与 pbxproj（xcodegen 按 glob 收文件，#117 不改变文件身份，无需重生成工程）。
- #120 与 #121 均触及 `Components/Button/`：`ButtonRoleStyleRole` 归 #120，其余 Button style 归 #121，需在 #121 启动前确认 #120 已合入。

### 外部

- SwiftUI iOS 26 / macOS 26 SDK：`ConcentricRectangle`、`Edge.Corner.Style.concentric`、`.glassEffect()`、`@Entry`。
- `ios-visual-reviewer` agent：Task #125 的判定方。
- 下游 any-writer：不阻塞本 epic，但 #126 须产出供其迁移的完整破坏清单。
...
- #117 / #118：各半天，纯删除 + 连带清理，机械但覆盖面广（易漏 `App/`、CI matrix 与快照 JSON）。
- #119 / #120：各 1 天。#120 更重——完整映射表 + macOS 降级验证 + accent 衍生族推导 + `ButtonRoleStyleRole` 重写。
- #121：1 天，编译器驱动，Sources 42 处改名 + 18 处 `*Font` + Tests 3 处 + 删别名。
- #122：1 天，本 epic 判断密度最高的一段，逐调用点确认换值。
- #123：半天到 1 天，触控与 Dynamic Type 断言。
- #124：半天到 1 天，23 个源文件的注释清理。
- #125：1 天，含最多 2 轮复审与修复。
- #126：1 天，文档、CI 复核、发版。

合计约 8–10 天。关键路径 #118 → #120 → #121 → #122 → #123 → #124 → #125 → #126；#117 与 #119 搭车并行，不在关键路径上。

## Tasks Created

- [ ] #117 - 删除 6 个 GitHub 专用组件 (parallel: true)
- [ ] #118 - 删除 Blossom trait 与 CoreGradient (parallel: true)
- [ ] #119 - 重铸字体 / 圆角 / 尺寸 / 阴影 token (parallel: true, depends_on: #118)
- [ ] #120 - 重铸语义色层与 accent 衍生族 (parallel: true, depends_on: #118)
- [ ] #121 - 组件机械改名扫尾并删除弃用别名 (parallel: false, depends_on: #119, #120)
- [ ] #122 - 同名换值逐点重审与迁移映射表 (parallel: false, depends_on: #121)
- [ ] #123 - 触控目标与 Dynamic Type 验证 (parallel: false, depends_on: #122)
- [ ] #124 - 清理注释 (parallel: false, depends_on: #123)
- [ ] #125 - 视觉终审与修复 (parallel: false, depends_on: #124)
- [ ] #126 - 文档、CI 与版本收尾 (parallel: false, depends_on: #125)

Total tasks: 10
Parallel tasks: 4（#117∥#118，随后 #119∥#120）
Sequential tasks: 6（#121 → #122 → #123 → #124 → #125 → #126）
