...
Sources 内需改名的 `.coreFont` 调用有 42 处，另有 18 处旧 `*Font` static var（App 宿主 17 + `Avatar.swift` 1），删 `CoreRadius.mediumPlus` 直接打断 `Sidebar.swift:157,411`。若 Task 003 硬改名，`swift build` 会从 003 一直红到迁移完成（约 36 小时），期间 CI 与每个 checkpoint 评审全部失去把关能力，且 003∥004 的并行声明失效——两者之一注定落在红树上无法独立验证。
...
- 005：1 天，编译器驱动，Sources 42 处改名 + 18 处 `*Font` + Tests 3 处 + 删别名。
