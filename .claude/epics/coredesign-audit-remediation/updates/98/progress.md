...
| 文件 | 处置 | 关键 |
|---|---|---|
| `StatusColorsTests` | 改写为真断言 | 5 个 `let _: Color`（0 `#expect`）→ 20 条 asset 名断言；反证：篡改某 token asset 名即红 |
...
- suites −1：删 Avatar/ProgressIndicator 两 suite，加 BlossomColorDivergence 一 suite（SurfaceKind 保留为空 suite——非-`@Test` 的编译期 case 守卫）。
