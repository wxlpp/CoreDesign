...
## CI iOS Simulator job 失败与平台自适应修复

首次 PR CI：SwiftPM ×2 + 下游 probe 绿，但 **iOS Simulator job 红**——`BlossomColorDivergenceTests` 的 `lightHex` 在 iOS 下返回 nil，真值断言 `nil == "#0077FA"` 失败。

**根因（#92 交接早已记录的坑，计划没预见到 CI iOS job 会跑这个新测试）**：xcodebuild 把 `.xcassets` 编译成 `Assets.car`，原始 `Contents.json` 目录不存在——正是 `BlossomAssetTests` 被 CI iOS job `-skip-testing` 的同一原因。而本测试是新建的、不在 skip 列表，且计划硬边界禁止改 CI skip 列表 / 只碰 `Tests/`（改 `ci.yml` 会越界）。

**修法（平台自适应，只改测试自身）**：加 `xcassetsIsRawDirectory` 检测 `.xcassets` 是否以原始目录分发——
- SwiftPM（目录存在）：真值断言照跑，`lightHex` 必须有值（SwiftPM 下的解析回退仍会红，reverse-proof 不被降级掩盖，已实测）。
- xcodebuild/Assets.car（目录不存在）：跳过真值断言，只保留 **asset 名分流断言**（两平台都强，`brand-5` vs `blossom-brand-5` 已区分分流）。

asset 名断言是 C4a 的核心（不同 asset = 不同颜色）；真 sRGB 值是 SwiftPM 下的加强层。两种分发都能区分 trait 分流。
