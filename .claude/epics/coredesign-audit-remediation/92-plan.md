...
```bash
# 必须 set -o pipefail:管道优先级高于 &&,`swift test | tail` 的退出码取 tail
# 恒为 0,测试变红不会中断链条、整条也返回 0——按退出码判绿会误判。
set -o pipefail
...
# 的 warning,期望为空。
# 不要在这之前跑 `swift build`——那会把 Sources/ 的 warning 编译掉并丢弃,
# 随后的 swift test 因模块 up-to-date 不再重放诊断,恰好漏掉本任务改的
# Tokens/*.swift 那一类。让 swift test 冷跑(它会顺带编译库)。
# 不要绑 session UUID 的 scratchpad 路径——若在另一个 session 执行,该目录不存在,
# tee/grep 会把错误写到 stderr 而 stdout 为空,「期望无输出」的判据于是静默通过,
# 即使真有新 warning。用自建目录并在用前断言日志非空。
LOGDIR="${TMPDIR:-/tmp}/coredesign-92"
mkdir -p "$LOGDIR"
swift package clean
swift test 2>&1 | tee "$LOGDIR/92-test.log" | tail -1
[ -s "$LOGDIR/92-test.log" ] || { echo "日志为空/未生成,判据无效"; exit 1; }
grep 'warning:' "$LOGDIR/92-test.log" | grep -v "is deprecated: Use SwiftUI ContentUnavailableView"
```

Expected: 最后一条 grep **无输出**（既有的 `EmptyState` deprecation 全部来自这一条 message，属 #6 范围）。若有任何输出，说明本任务引入了新 warning，须处理。

> **本步只看 grep 结果**。这次 `swift test` 冷跑同样可能撞上 ToastHost flake，`tail -1` 显示 failed 不代表 warning 判据失效——测试红绿以 Step 7 为准，遇 flake 按 Global Constraints 的规则重跑。
...
- [ ] **Step 2: 立刻提交 C9a（不要拖到 Task 末尾）**

必须现在提交。后面路径 3 失败时要 `git checkout -- App/project.yml` 回滚，而 Task 3 原本第一次 commit 在末尾的 Step 9——那时 C9a 的修改还在工作区未提交状态，会被一起丢掉。更隐蔽的是 Step 7 的「已跑过 generate 就跳过」判断会让执行者绕过补救，最终 C9a 既没进 spec 也没进生成物，Task 6 却把它标成「已修复」。

```bash
git add App/project.yml
git commit -m "Issue #92: fix preview host xcodeVersion (C9a)"
```

- [ ] **Step 3: 确认 SWIFT_VERSION 与主包一致**

`App/project.yml` 两处 `SWIFT_VERSION: "6.0"` 与主包 `swiftLanguageModes: [.v6]` 语义一致（都是 Swift 6 语言模式），**不需要改**。此步骤只是确认，不产生改动。

- [ ] **Step 4: 探明 Blossom 在预览宿主中的可行启用方式（C9b）**
...
git checkout -- App/CoreDesignPreview.xcodeproj/xcshareddata/   # regenerate 必然删掉 shared scheme,恢复
ls App/CoreDesignPreview.xcodeproj/xcshareddata/xcschemes/      # 确认已恢复
xcodebuild build -project App/CoreDesignPreview.xcodeproj -scheme CoreDesignPreview \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' \
  CODE_SIGNING_ALLOWED=NO 2>&1 | tail -5
```

**若颜色未变（预期如此）**，先回滚这处编辑再进 Step 5 的路径 4，否则一条已知无效且语义误导的编译条件会被 Step 9 提交进预览宿主：

```bash
git checkout -- App/project.yml App/CoreDesignPreview.xcodeproj
```
...
- [ ] **Step 5: 路径 4——wrapper 本地包（前三条不可行时必须尝试，不可跳过）**
...
>
> **注意该验证的环境前提**：初次实测是在根目录名为 `CoreDesign` 的 checkout 中做的。在 worktree（目录名 `issue-92-build-config`）中**必须**用下面写法里的 `.package(name:path:traits:)` 形式，否则解析失败——这一点也已实测确认。
...
let package = Package(
    ...
    dependencies: [
        // 必须显式写 name:——SwiftPM 对 path 依赖的 identity 取**目录 basename**,
        // 而本任务在 worktree 里执行,根目录名是 issue-92-build-config 而非 CoreDesign。
        // 不写 name: 会报 unknown package 'CoreDesign'; valid packages are: 'issue-92-build-config'。
        // 显式命名后 identity 与引用名解耦,worktree 与正式 checkout 两侧都成立。
        .package(name: "CoreDesign", path: "../..", traits: ["Blossom"]),
    ],
    targets: [
        .target(
            name: "BlossomPreviewShim",
...
> **这个改动的代价必须记进 `ci-decision.md`**：换掉 `packages:` 里的 CoreDesign 意味着预览宿主之后**只能渲染 Blossom**，失去看默认 Craft 主题的能力——而它目前是团队看默认主题的唯一 App 入口。92.md 的 C9b 只要求「能渲染 Blossom」，字面达成，但这是一处未经讨论的能力损失。须在 `ci-decision.md` 写明还原路径（改 shim 的 `traits:` 后重跑 `xcodegen generate`），并让后续 Issue 的视觉评审知道宿主当前是什么主题。
>
> **临时 test target 的去留**：下面用来断言 trait 生效的 test target，验证完**删除**（Step 9 会 `git add App/BlossomPreviewShim` 整目录，不删就会被提交成一个自称临时的常驻物）。若决定保留为回归守护，须在 `ci-decision.md` 说明它不进 CI——shim 不在根包 graph 内，`swift test` 跑不到它。

验证（必须看真实 asset 解析结果，不能只看构建成功——这正是 xcodegen `traits:` 的陷阱）：

先用 SwiftPM 层面确认 trait 生效（最快、无需 simulator）。给上面的 `Package.swift` 的 `targets:` 追加一个**临时** test target：

```swift
        .testTarget(name: "BlossomPreviewShimTests", dependencies: ["BlossomPreviewShim"]),
```

创建 `App/BlossomPreviewShim/Tests/BlossomPreviewShimTests/TraitProbe.swift`：

```swift
import Testing
import SwiftUI
@testable import BlossomPreviewShim

@Suite("Blossom trait probe")
struct TraitProbe {
    @Test("accent 指向 Blossom 色板")
    func accentUsesBlossomAsset() {
        let description = String(describing: Color.accent)
        // 默认 trait 下这里会是 "brand-5"；Blossom 生效时是 "blossom-brand-5"
        #expect(description.contains("blossom-brand-5"), "实际解析到：\(description)")
    }
}
```
...
```bash
cd App/BlossomPreviewShim && swift test 2>&1 | tail -3 && cd ../..
```

Expected: `Test run with 1 test in 1 suite passed`。若断言失败，输出会打印实际解析到的 asset 名——若是 `brand-5` 说明 trait 未生效，本路径失败。

**验证通过后删除该 test target 与其目录**（Step 9 会 `git add App/BlossomPreviewShim` 整目录，不删就会把一个自称临时的东西提交成常驻物）：

```bash
rm -rf App/BlossomPreviewShim/Tests
# 并从 Package.swift 的 targets: 中移除那行 .testTarget
```
...
- [ ] **Step 7: 让 C9a 的修正真正落地**

**跳过判据（不能只看「跑过 generate」）**：确认 `App/project.yml:6` 当前确为 `26.0`，**且** `.xcodeproj` 是在该值生效之后生成的。若路径 3 失败并回滚过，spec 可能已回到 commit 状态而生成物是旧的——此时不可跳过。否则：`xcodeVersion: "26.0"` 只改了 spec 文件，已提交的 `.xcodeproj` 仍是按旧值生成的。四条 SwiftPM 命令不触碰 `App/`，所以不会暴露这一点。二选一并写明选了哪个：
...
- [ ] **Step 9: Commit（含本 Task 产生的全部落盘物）**

```bash
git add App/project.yml .claude/epics/coredesign-audit-remediation/updates/92/ci-decision.md
# 分开写并各自守卫——`git add a b` 在其中一个 pathspec 不存在时整条失败且
# 一个文件都不暂存(实测),配上 `2>/dev/null || true` 会完全无声。
[ -d App/BlossomPreviewShim ] && git add App/BlossomPreviewShim
[ -d App/CoreDesignPreview.xcodeproj ] && git add App/CoreDesignPreview.xcodeproj
git status --short          # 确认工作区无遗留未提交文件(跨 Task 脏状态对 subagent 执行是隐患)
git commit -m "Issue #92: land Blossom trait decision for preview host"
```
...
先建目录（`.github/` 在工作区**根本不存在**，不是空目录——直接 heredoc 写文件会失败）：
...
on:
  push:
    # 用 branches 而非 paths:Step 2 是该分支的首次 push,GitHub 对新建分支的
    # paths diff 计算行为不确定;而探针在 Task 4 Step 5 就删掉,paths 过滤无收益。
    branches: ['issue-**']
...
jobs:
  probe:
    ...
    # 不计入(排队上限 24h),所以它挡不住「label 不可用导致长期 queued」——那种
    # 情况靠 Step 3 的轮询上限兜底。
    timeout-minutes: 20
    steps:
      - name: Runner image
...
push 即触发（Step 2 已 push）。run 注册有延迟，先轮询拿 id：

```bash
for i in $(seq 1 12); do
  run_id=$(gh run list --branch issue-92-build-config --limit 5 \
    --json databaseId,name -q '[.[]|select(.name=="_probe-runner")][0].databaseId')
  [ -n "$run_id" ] && break
  sleep 10
done
[ -n "$run_id" ] || { echo "探针未触发,检查 branches 过滤与推送是否成功"; exit 1; }
gh run watch "$run_id"
      ...
      - name: Test
        ...
        run: |
          ...
          run_tests 2>&1 | tee first.log || {
            # 只有当失败用例**全部**落在已知 flake 的两个 ToastHost 用例上才允许重跑。
            # 无差别重试会把本任务可能引入的真实并发时序回归一起洗白——那类 bug
            # 的表现形式与 timing flake 完全一致。
            # 注意:下面的匹配串须用 Task 1 Step 7 的真实输出校准,不要照抄。
            # 锚点**不能**用 ✘/✔ 符号:xcodebuild 日志用 ✘(U+2718),而 swift test
            # 用 SF Symbol(􁁛),同一条正则在另一侧必然失效。且 ✘ 还会命中
            # 「Test run with N tests ... failed」汇总行,它既不含 dismiss(id:)
            # 也不含 double-fire,会让负向 grep 无条件命中、白名单永久判红。
            # 改用符号无关的文本锚点(两侧输出都含):
            # 必须先落到变量、断开管道:`grep -q` 命中首行即退出并向上游发 SIGPIPE,
            # 上游以 141 终止,而本 step 开头的 `set -o pipefail` 会把 141 提升为整条
            # 管道的退出码 → if 恒假 → 一切失败都走「重跑」。实测「flake + 真实回归」
            # 场景会被误判为「仅 flake」,正是白名单要防的那件事。
    ...
    steps:
      ...
      - name: Run iOS tests
        ...
        run: |
          ...
          run_ios 2>&1 | tee ios-first.log || {
            # 同样收紧到白名单。xcodebuild 的失败模式更杂(simulator boot 超时等),
            # 无差别重试比 SwiftPM 侧更危险。
            # 锚点**不能**用 ✘/✔ 符号:xcodebuild 日志用 ✘(U+2718),而 swift test
            # 用 SF Symbol(􁁛),同一条正则在另一侧必然失效。且 ✘ 还会命中
            # 「Test run with N tests ... failed」汇总行,它既不含 dismiss(id:)
            # 也不含 double-fire,会让负向 grep 无条件命中、白名单永久判红。
            # 改用符号无关的文本锚点(两侧输出都含):
            # 同 swiftpm job:先落变量断开管道,避免 grep -q 的 SIGPIPE 被 pipefail
            # 提升为整条管道退出码而使 if 恒假。
            ...
            echo "仅已知 flake 失败,重跑一次"
            run_ios 2>&1 | tee ios-second.log
          }
      - name: Upload test logs
        if: always()
        uses: actions/upload-artifact@v4
...
`ci.yml` 此刻还是 untracked，直接 `git push` 推不上去，随后 `gh run list` 会返回空、`run_id` 为空、`gh run watch ""` 报错。必须先提交：

```bash
[ -f .github/workflows/ci.yml ] && git add .github/workflows/ci.yml
[ -f scripts/pre-push-check.sh ] && git add scripts/pre-push-check.sh
git add .claude/epics/coredesign-audit-remediation/updates/92/ci-decision.md
git status --short          # 确认该进的都进了
git commit -m "Issue #92: land CI gate at determined fallback level"
git push
```

> 分开写两条 `git add` 并各自用 `[ -f ]` 守卫，是因为 `git add a b` 在其中一个 pathspec 不存在时**整条命令失败且一个文件都不暂存**（实测：`git add missing real.txt` 后暂存区为空）。级别 4 时 `ci.yml` 不存在，写成一条会连 `ci-decision.md` 一起丢掉。

- [ ] **Step 3: 验证门禁真的跑起来了**

级别 1–3（加轮询兜底，run 注册有延迟）：

```bash
for i in $(seq 1 12); do
  run_id=$(gh run list --branch issue-92-build-config --limit 5 \
    --json databaseId,name -q '[.[]|select(.name=="CI")][0].databaseId')
  [ -n "$run_id" ] && break
  sleep 10
done
[ -n "$run_id" ] || { echo "CI 未触发,检查 push filter 是否含 issue-**"; exit 1; }
gh run watch "$run_id"
```
...
Expected: 打印到「全部通过」。**该脚本不会自动生效**——`.git/hooks/pre-push` 不可提交，须在 `ci-decision.md` 里写明安装指令：

```bash
# 用路径无关写法——本任务在 worktree 中,那里的 .git 是**文件**(gitdir 指针)
# 而非目录,且 worktree 的 hooks 默认指向 $GIT_COMMON_DIR/hooks。
# **须在主 checkout 中执行,不要在 worktree 里** ——hooks 目录是所有 worktree 共享的,
# 而 --show-toplevel 在 worktree 里解析到 .worktrees/... ,该目录收尾后会被删除,
# 链接随之断掉。
ln -sf "$(git rev-parse --show-toplevel)/scripts/pre-push-check.sh" \
...
LOGDIR="${TMPDIR:-/tmp}/coredesign-92"
mkdir -p "$LOGDIR"
set -o pipefail   # 否则 `cmd | tail` 的退出码取 tail,恒为 0,红了也看不出来

# 第一段:四条命令的通过性。逐条跑、不串 && ——串起来管道会吞掉退出码。
swift build
swift test
swift build --traits Blossom
swift test --traits Blossom

# 第二段:warning 采集。**两侧都必须 clean 后冷跑**——若前面已 build 过,库源码的
# warning 会被丢弃且不再重放(正是 Task 1 Step 8 注释里的陷阱)。
swift package clean && swift test 2>&1 | tee "$LOGDIR/92-final.log" | tail -1
[ -s "$LOGDIR/92-final.log" ] || { echo "日志为空/未生成,判据无效"; exit 1; }
grep 'warning:' "$LOGDIR/92-final.log" | grep -v "is deprecated: Use SwiftUI ContentUnavailableView"   # 期望无输出

swift package clean && swift test --traits Blossom 2>&1 | tee "$LOGDIR/92-final-b.log" | tail -1
[ -s "$LOGDIR/92-final-b.log" ] || { echo "日志为空/未生成,判据无效"; exit 1; }
grep 'warning:' "$LOGDIR/92-final-b.log" | grep -v "is deprecated: Use SwiftUI ContentUnavailableView"   # 期望无输出
```
