...
```bash
swift build 2>&1 | grep 'error:' | sed 's|.*/CoreDesign/||' | sort -u
```
...
```bash
swift test 2>&1 | grep -c 'warning:'
```

Expected: `12`（全部是既有的 `EmptyState` deprecation，属 #6 范围）。若 > 12，说明本任务引入了新 warning，须处理。
...
- [ ] **Step 4: 若三条路径均不可行，记录为未完成并说明**

C9b 若无法达成，在 `ci-decision.md` 中写明：预览宿主无法渲染 Blossom，视觉验证 Blossom 只能靠 Xcode canvas 里手动切 trait 或库内 `#Preview`。`audit-checklist.md` 的 C9b 标为「记录不修 + 理由：xcodegen 不支持 traits，且无等价替代」，并把这条从 SC-7 的「已修复」计数中排除。**不要为达成它而手工编辑 `.pbxproj`**——下次 `xcodegen generate` 会覆盖掉。

- [ ] **Step 5: 验证四条 SwiftPM 命令未受影响**

```bash
swift build && swift test 2>&1 | tail -1 && swift build --traits Blossom && swift test --traits Blossom 2>&1 | tail -1
```

Expected: 与 Task 1 Step 7 相同（`App/` 的改动不影响 SwiftPM 构建）

- [ ] **Step 6: Commit**

```bash
git add App/project.yml
git status --short App/    # 确认没有意外提交重新生成的 .xcodeproj
git commit -m "Issue #92: fix preview host xcodeVersion; record Blossom trait findings"
```
...
- Create: `.claude/epics/coredesign-audit-remediation/updates/92/ci-decision.md`
...
```bash
gh workflow run _probe-runner.yml --ref issue-92-build-config
sleep 30
gh run list --workflow=_probe-runner.yml --limit 1
```
...
echo "3/4 swift build --traits Blossom"
swift build --traits Blossom

echo "4/4 swift test --traits Blossom"
swift test --traits Blossom
...
swift package clean
swift build && swift test 2>&1 | tail -1 && swift build --traits Blossom && swift test --traits Blossom 2>&1 | tail -1
swift test 2>&1 | grep -c 'warning:'   # 期望 12,不多不少
```
