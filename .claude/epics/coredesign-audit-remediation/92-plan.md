...
LOGDIR=/private/tmp/claude-501/-Users-evan-Repositories-work-spec-CoreDesign/773d095b-9e7d-4715-9687-1768f95c7dc3/scratchpad
swift package clean
...
**若颜色未变（预期如此）**，先回滚这处编辑再进 Step 5 的路径 4，否则一条已知无效且语义误导的编译条件会被 Step 8 提交进预览宿主：
...
>
> **临时 test target 的去留**：下面用来断言 trait 生效的 test target，验证完**删除**（Step 8 会 `git add App/BlossomPreviewShim` 整目录，不删就会被提交成一个自称临时的常驻物）。若决定保留为回归守护，须在 `ci-decision.md` 说明它不进 CI——shim 不在根包 graph 内，`swift test` 跑不到它。
      ...
      - name: Test
        ...
        run: |
          ...
          run_tests 2>&1 | tee first.log || {
            ...
            # 改用符号无关的文本锚点(两侧输出都含):
            if { grep 'recorded an issue' first.log
                 grep -E 'Test "[^"]*" failed after' first.log
               } | grep -qvE 'dismiss\(id:\)|double-fire'; then
      ...
      - name: Run iOS tests
        ...
        run: |
          ...
          run_ios 2>&1 | tee ios-first.log || {
            ...
            # 也不含 double-fire,会让负向 grep 无条件命中、白名单永久判红。
            # 改用符号无关的文本锚点(两侧输出都含):
            if { grep 'recorded an issue' ios-first.log
