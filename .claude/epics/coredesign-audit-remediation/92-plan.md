...
swift package clean
swift test 2>&1 | tee /tmp/92-test.log | tail -1
grep 'warning:' /tmp/92-test.log | grep -v "is deprecated: Use SwiftUI ContentUnavailableView"
```
...
**若颜色未变（预期如此）**，先回滚这处编辑再进路径 4，否则一条已知无效且语义误导的编译条件会被 Step 8 提交进预览宿主：
...
**若前面的路径 3 / 路径 4 已经跑过 `xcodegen generate`，本步骤的 (a) 已经完成，只需确认并跳过。** 否则：`xcodeVersion: "26.0"` 只改了 spec 文件，已提交的 `.xcodeproj` 仍是按旧值生成的。四条 SwiftPM 命令不触碰 `App/`，所以不会暴露这一点。二选一并写明选了哪个：
...
jobs:
  probe:
    ...
    # 若某个 label 在本组织不可用,job 会长时间排队,gh run watch 会一直挂住
    timeout-minutes: 20
    steps:
      - name: Runner image
        run: sw_vers && uname -m
      ...
      - name: Test
        ...
        run: |
          ...
          run_tests 2>&1 | tee first.log || {
            ...
            if grep -E 'recorded an issue|✘ Test ' first.log \
      ...
      - name: Run iOS tests
        ...
        run: |
          ...
          run_ios 2>&1 | tee ios-first.log || {
            ...
            # 无差别重试比 SwiftPM 侧更危险。
            if grep -E 'recorded an issue|✘ Test ' ios-first.log \
               | grep -qvE 'dismiss\(id:\)|double-fire'; then
              echo "存在已知 flake 之外的失败 → 直接判红"; exit 1
            fi
            echo "仅已知 flake 失败,重跑一次"
...
swift package clean
swift build && swift test 2>&1 | tail -1 && swift build --traits Blossom && swift test --traits Blossom 2>&1 | tail -1
swift test 2>&1 | tee /tmp/92-final.log | tail -1
grep 'warning:' /tmp/92-final.log | grep -v "is deprecated: Use SwiftUI ContentUnavailableView"   # 期望无输出
# Blossom 侧也查一遍——上面只覆盖默认 trait,#if Blossom 分支下的新 warning 抓不到
swift package clean && swift test --traits Blossom 2>&1 | tee /tmp/92-final-b.log | tail -1
grep 'warning:' /tmp/92-final-b.log | grep -v "is deprecated: Use SwiftUI ContentUnavailableView"   # 期望无输出
```
