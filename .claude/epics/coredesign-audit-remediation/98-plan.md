...
**处置：整文件删除。** `SurfaceKind` 的枚举 case 存在性由编译器保证（组件用它就会编译失败），无需测试守护；其 token 映射的真行为已由消费它的组件测试（如 `CommentCardTests` 的 `.surface(.card)`）间接覆盖。记入 C5 清单「已删除，映射从 Tests/ 不可断言」。
...
>
> 另：`audit-checklist.md:19` 与 `98.md:16` 的基线仍写「96 tests」，实际是 **104**（前序 Issue 增加）——本任务的 PR 文案用 104，不要传播旧数字。
