...
- **越界红线**：本任务只允许改动这些文件——`Package.swift`、`Sources/CoreDesign/Resources/Localizable.xcstrings`（新增）、`Toast.swift`、`CoreMenuButton.swift`（在 `Components/BottomInputBar/` 下）、`BottomInputBar.swift`（在 `Components/BottomInputBar/` 下，#99 的 Suggestions/Send/Stop）、`Form.swift`（#99 的 DangerIcon Alert）、`BookCover.swift`、`ProgressBar.swift`、`ProgressIndicator.swift`、`Tag.swift`、`AvatarGroup.swift`、`AsyncButton.swift`（在 `Components/Button/` 下，复用 Loading key）、`.claude/` 下的追踪文件（audit-checklist / progress）。**不碰任何其他 Sources / Tests**。
...
- **范围：13 个组件内部 UI 串**（原 8 + #99 扩入的 4 + AsyncButton 复用 Loading）。`#Preview` 里的演示文案（书名 / tag 名等）是示例数据，**不本地化**。CommentCard / SearchField 的英文 a11y 串**不在本任务范围**（那些串含动态插值 `\(author)` / `\(placeholder)`，非静态字面量）。**`AsyncButton` 的 `accessibilityValue(Text("Loading"))`（`:153`）纳入本任务**——它与 `ProgressIndicator` 现已本地化的 `"Loading"` 是同一静态字面量、复用同 key，留它不本地化正是 D2 要消除的一文件之隔的口径不一致（评审 Suggestion 2）。**#99 扩入（主编排裁决）**：#99 新增的 4 处英文 a11y 串（`BottomInputBar` Suggestions/Send/Stop、`Form.DangerIcon` Alert）本次一并纳入 catalog，兑现 D2 行 #99 指针，形态同「已英文串纳入 bundle」（见 Task 3）。
- **key = 英文源串本身**。11 个唯一 key：`Untitled` / `Tap to dismiss` / `Menu` / `Loading` / `Progress` / `Remove tag` / `%lld more avatars` / `Suggestions` / `Send` / `Stop` / `Alert`（`Menu` 两处复用同一 key → 11 key / 13 调用点）。
...
## 13 处目标串一览（侦察已核对，行号为写计划时快照，执行时按符号定位）
...
| 13 | `AsyncButton.swift:153` | `.accessibilityValue(Text("Loading"))` | 已英，纳入 catalog（复用 `Loading` key，评审 Suggestion 2） | `Loading`（复用） |
...
- 若 `probeIsWired` FAIL 且实际值为 `"l10n.spike.probe"` → **打包 wiring 没生效**（SwiftPM CLI `swift build`/`swift test` 未编译/解析 `.xcstrings`——#98 的 `.xcassets` 在 SwiftPM 下不编译成 `.car` 是同类坑）。先排查 `defaultLocalization` 是否落对、`.xcstrings` 是否在 `Resources/` 下、JSON 是否合法。
  - **Fallback（若排查后 `.xcstrings` 确实不被 SwiftPM CLI 解析）**：改用传统 `Resources/en.lproj/Localizable.strings`（`%lld more avatars` 用 `Localizable.stringsdict`），重跑 spike。
  - **仍 FAIL → 升级给 orchestrate，不要硬猜继续。**
  - **关键——AC 比 spike 松，不得靠 AC 蒙混**：本任务 AC（`100.md:33,36`）只要求 `.xcstrings` 文件存在 + 四命令绿，**不**要求运行期真解析。而每个 en key == 英文值，运行期回落不可见——**AC 可被满足而 catalog 是空壳（wiring 没接上、本地化能力没真建立）**。spike 是唯一能证明 wiring 的门。**spike 未过就把 D2 标「已修复」= 假审计记录**，禁止。**Task 2/3 只在 spike 过之后进行。**
...
## Task 3: 9 处已英文串纳入 bundle 形态（含 ProgressBar / AvatarGroup 的 tricky 形态 + #99 的 BottomInputBar/Form a11y 串 + AsyncButton 复用 Loading）
...
- [ ] **Step 6b: AsyncButton — `accessibilityValue(Text("Loading"))` 走 bundle（复用 `Loading` key，评审 Suggestion 2）**

`AsyncButton.swift:153` 的 `content.accessibilityValue(Text("Loading"))` 与 `ProgressIndicator` 的 `"Loading"` 是同一静态串——复用同 `Loading` key，不新增 catalog 条目，只补 `bundle: .module`。

Before（`AsyncButton.swift` 第 153 行）:
...
**判据 C（已英文串纳入核对——9 处已英文串须走 `bundle: .module`，无裸字面量 a11y label / value 残留）**：

改前这 9 处英文串（原 4 处 + #99 扩入的 4 处 + AsyncButton 复用 Loading）以裸字面量 / 无 bundle 的 `Text` 形式存在。改后全部应为 `Text(..., bundle: .module)` / `String(localized:bundle:)` 形态。应走 `bundle: .module` 的核对清单（逐项确认）：
...
- **AsyncButton**：`accessibilityValue(Text("Loading"))` 复用 `Loading` key（注意是 `accessibilityValue` 而非 label）

判定命令（这些字符串**不应**再以裸字面量或无 bundle 的 `Text` 出现）：

```bash
grep -rn '\.accessibilityLabel("Loading")\|\.accessibilityLabel("Suggestions")\|\.accessibilityLabel("Send")\|\.accessibilityLabel("Stop")\|\.accessibilityLabel("Alert")\|\.accessibilityLabel(Text("Remove tag"))\|\.accessibilityLabel(self.label ?? "Progress")\|\.accessibilityValue(Text("Loading"))' Sources/CoreDesign --include='*.swift'
```

Expected: **零输出**（退出码 1）。改前 `ProgressIndicator:34` / `Tag:110` / `ProgressBar:57` / `BottomInputBar:152,168,181` / `Form:115` / `AsyncButton:153` 共 8 命中（`AvatarGroup` 走 `overflowLabel` 函数体、不在此 grep 形态内，单独由 AvatarGroupTests + 判据留意）；改后全清零，证明均已改走 `Text(..., bundle: .module)`。若仍有命中 → 有已英文串漏改 bundle 形态。

- [ ] **Step 3: 越界核查（改动仅限白名单文件；Package.swift 只多一行）**

Run:

用**反向自查**（列出白名单**外**的改动，能暴露误改 `App/` 等盲区，优于只看白名单内的 `--stat`）：

```bash
git diff --name-only epic/coredesign-audit-remediation..HEAD \
  | grep -vE '^(Sources/CoreDesign/Components/(BookCover|Toast|BottomInputBar|ProgressBar|ProgressIndicator|Tag|AvatarGroup|Form|Button)/|Sources/CoreDesign/Resources/Localizable\.xcstrings|Package\.swift|\.claude/)'
echo "rc=$?"
git diff epic/coredesign-audit-remediation..HEAD -- Package.swift
```

Expected:
- **`rc=1`、无输出**——所有改动落在白名单内（10 个组件源文件含 `AsyncButton`、`Localizable.xcstrings`、`Package.swift`、`.claude/`），`App/`（仓库存在 `App/Tests` 等）与其它 Sources / Tests 均未触碰（AvatarGroupTests 未动）。
  - **注（评审 Suggestion E）**：白名单是**目录级**（`Button/`、`BottomInputBar/` 等含其它 sibling 文件）——换取了 `App/` 越界覆盖，代价是同目录 sibling 文件的误改不被此反向 guard 捕获。本任务逐 step 的 `git add` 是 file-precise 的（只 add 明确文件），故实际风险低；若要双保险，另跑一条 file-level `git diff --stat -- <逐个文件路径>` 确认无计划外文件。
...
- 新增 `Sources/CoreDesign/Resources/Localizable.xcstrings`（sourceLanguage=en，11 唯一 key，`Menu` 两处复用 → 13 调用点）。
...
- 中文判据 A（`"未命名"|"点击关闭"|"菜单"` 引号形式）清零；判据 B 残余中文全为 `#Preview` 演示数据 / 行尾注释；判据 C（9 处已英文串裸字面量 a11y label 及 value）清零。
...
- [ ] **finishing-a-development-branch（Option 2：开 PR，不合并）**：base = `epic/coredesign-audit-remediation`（**不是 main**）。PR body 覆盖：承载 D2；13 串 catalog 化（含 #99 扩入的 4 处 + AsyncButton 复用 Loading）+ `defaultLocalization`；四命令冷跑绿证据；判据 A/B/C；#99 指针已兑现（主编排裁决扩范围）。PR body 末尾附：
...
**5. 一致性核查（11 key ⇄ 13 调用点）**：11 唯一 key（`Menu` 复用）对 13 调用点——4 中文（BookCover/Toast/CoreMenuButton×2）+ 4 已英文（ProgressIndicator/ProgressBar/Tag/AvatarGroup）+ 4 个 #99（BottomInputBar×3/Form）+ AsyncButton 复用 Loading。越界白名单含全部触及文件（10 swift + `Localizable.xcstrings` + `Package.swift`）。✅
