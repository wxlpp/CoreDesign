...
- **第 5 条命令是本任务独有的硬性验收**：`xcodebuild test -scheme CoreDesignPreview -destination 'platform=iOS Simulator,name=iPhone 17 Pro'`，且布局断言必须在其下**真正执行**（测试数 > 0，非空转）。
...
for t in displayLarge titleLarge titleMedium titleSmall subtitle bodyLarge bodyMedium bodySmall caption captionSmall; do
  n=$(grep -rn "CoreTypography.${t}Font\|CoreTypography.${t}LineSpacing\|CoreTypography.${t}Tracking" Sources/CoreDesign/Components Sources/CoreDesign/Modifier --include='*.swift' | wc -l | tr -d ' ')
  echo "$t: $n 处剩余"
done
```

零引用的常量**删除**（#97 刚清完死代码，不留新的）。仍有引用的（情况 3）保留并记录原因。
...
struct DynamicTypeLayoutTests {
    ...
    func sidebarRowsGrowWithDynamicType() {
        ...
        // 内容固有高度下限：至少容纳一行放大后的文字。
        #expect(ax5 > 40, "accessibility5 下高度未超过 minHeight 40——内容被裁")
    }
...
```
Expected: `TEST SUCCEEDED`，且 `DynamicTypeLayoutTests` 的 3 个用例**在计数内**（`xcodebuild` 输出里能看到它们跑了——**非空转**）。

> ⚠️ **CI 的 simulator job 有 skip 列表**（`BlossomAssetTests` / `ToastHostTests`）。新增的 `DynamicTypeLayoutTests` **不能**被误加进 skip——否则第 5 条命令绿但本层空转。检查 `.github/workflows/ci.yml` 的 `-skip-testing` 参数。

跑完 `git checkout -- App/CoreDesignPreview.xcodeproj/project.pbxproj` 回退 xcodegen 噪音（#96 踩过）。
...
Task 5 Step 2 的 `xcodebuild`。`TEST SUCCEEDED` 且布局断言在计数内。
