...
| C7a | ✅ **已修复**（GitHub #92）——两个 target 均启用 `.defaultIsolation(MainActor.self)`；fallout 用 `nonisolated` 处理（2 处库内 + 9 个公开 Sendable 类型，后者由 checkpoint 评审发现）。原缺陷：`Package.swift` 无 `swiftSettings`，未启用 `defaultIsolation` | `Package.swift` | FR-6 | #1 |
