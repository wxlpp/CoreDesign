...
| B9a | ✅ **已修复**（GitHub #97）——解码移到 `@State private var decodedImage` + `.task(id: data)`，不再每次 body 求值重解码。未用 init 解码（View 的 init 可能被频繁调用）。原缺陷：`BookCover` 在 body 里做 `UIImage`/`NSImage` 解码，列表滚动反复解码 | `BookCover.swift:74,95-106` | FR-5 | #6 |
...
| B9g | ✅ **已修复**（GitHub #97）——整文件删除（237 行）及其自证测试；`docs/README.md` 索引行与 `docs/components/empty-state.md` 一并清理。基线的 12 条 warning 随之归零。原缺陷：`EmptyState.swift` 237 行全文件已废弃，同一 deprecated message 重复 6 次 | `EmptyState.swift:54,141,153,182,203,216` | FR-5 | #6 |
