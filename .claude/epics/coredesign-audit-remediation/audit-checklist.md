...
| B2a | ✅ **已修复**（GitHub #95）——`CoreTypography.Token` 枚举 + `.coreFont(_:)` modifier（`@ScaledMetric` 缩放 size 与 lineSpacing），`relativeTo:` 从 0 → 2；旧 `*Font` 常量改从 `Token.spec` 派生（单一真值源，保留 Canvas 用途的 `titleLargeFont`）。**9 缩放 + captionSmall 固定**（经用户确认，见 NFR 例外 5）。iOS Simulator 布局断言实测通电。原缺陷：全部 10 个 typography token 用 `.system(size:)`，`relativeTo:` 出现 0 次 | `CoreTypography.swift:53-183` | FR-3 | #4 |
...
| B8b | ✅ **已修复**（GitHub #101）——两个 makeBody 抽出 file-private bannerBody(configuration:bordered:)，唯一差异 .bordered 由布尔控制。原缺陷：两个 `BannerStyle.makeBody` 11 行里只有最后一行不同 | `Banner.swift:177-192,210-225` | FR-4 | #10 |
...
| B8e | ✅ **已修复**（GitHub #101）——ToastLevel + MessageLevel 合并为单一 StatusLevel（nonisolated Sendable Equatable，info/success/warning/danger），Toast/Banner/probe/docs 同步；ToastItem: Sendable 与 downstream probe 仍绿。原缺陷：`ToastLevel` 与 `MessageLevel` case 集合完全相同却是两个类型 | `Toast.swift:24-29`、`Banner.swift:32-37` | FR-4 | #10 |
| B8f | ✅ **已修复**（GitHub #101）——StateLabel 3 switch 与 StatusRow 3 switch 各收敛为单个 @MainActor var spec（返回 icon/背景/文案 三元组），新增 case 只在一处要求穷举。原缺陷：各组件带 2–4 个平行 switch（`StateLabel` 4 个、`StatusRow` 3 个） | `StateLabel.swift:65-109`、`StatusRow.swift:66-91` | FR-4 | #10 |
| B8g | ✅ **已修复**（GitHub #101）——两处 glass overlay 抽为 segmentedGlassChrome(_:)（透明填充+交互玻璃+细描边），thumb 侧再叠 coreShadow。原缺陷：`SegmentedControl` 两处 glass 分支各自复制 overlay | `SegmentedControl.swift:121-147,379-409` | FR-4 | #10 |
...
| C2 | ✅ **已修复**（GitHub #98）——五个恒真文件按「改写优先于删除，改写必反证」处置：`StatusColorsTests`（→ 20 条 asset 名断言）、`FloatingGlassModifierTests`（→ `isInteractive` 默认/透传契约）改写为真断言并反证通过；`SurfaceKindTests` 断言瘦身（删恒真 `.count`，保留非-`@Test` 的编译期 case 守卫）；`AvatarTests`/`ProgressIndicatorTests` 删除（派生行为 body 内联，Tests/ 不可断言）。顺带清理 `CoreGradientTests` 的 `#expect(Bool(true))` 恒真占位（改写为 trait 分流断言）。逐文件清单见文末附录。原缺陷：约 3/4 测试是恒真断言（编译通过即必过） | `ProgressIndicatorTests`、`FloatingGlassModifierTests`、`StatusColorsTests`、`SurfaceKindTests`、`AvatarTests` | FR-6 | #7 |
...
| C4a | ✅ **已修复**（GitHub #98）——新增 `BlossomColorDivergenceTests`：`String(describing: Color.accent)` 取 asset 名 + 解析 `colorset/Contents.json` 的 light sRGB 分量，按 `#if Blossom` 断言不同真颜色值（默认 `brand-5`/`#0077FA`，Blossom `blossom-brand-5`/`#FF6F8E`）。两 trait 都跑都绿、断言值不同；反证（`swift package clean` 后篡改 Contents.json）确认断言真读了资源、非恒真。原缺陷：Blossom trait 核心行为无测试：`--traits Blossom` 与默认跑同样断言 | `swift test --traits Blossom` 输出 | FR-6 | #7 |
| C4b | ✅ **已修复**（GitHub #98）——`BlossomAssetTests` 新增 `gradientDepColorsetsPresent`，用 FileManager 覆盖 `violet-0…9` + `cyan-1` 的 `.colorset` 目录存在性（AC 指定 `violet-0…9` 全覆盖；实测真实消费点更窄——`CoreGradient.canvas` 用 violet-2+cyan-1、`secondaryAccent` 用 violet-5/6/7，注释已标注）。原缺陷：现有 Blossom asset guard 未覆盖分流同样依赖的 `violet-0…9` 与 `cyan-1` | `CoreDesignTests.swift:21-37` | FR-6 | #7 |
| C5 | ✅ **已修复**（GitHub #98）——产出逐文件测试处置清单（文末附录），覆盖 C2 五文件 + 约 15 个零测试目标，各标「本轮补测 / 已改写 / 记录不补 + 理由 / 已由 #97 删除」。AC 明确不要求全补（清单本身即交付物，是 SC-5 机械判定的前提）。原缺陷：零测试文件清单 | `ls Tests/CoreDesignTests/` | FR-6 | #7 |
| C6a | ✅ **已修复**（epic 收尾，经用户裁决）——README 的 `.package(…, branch: "main")` 改为 `from: "0.2.0"`（pin 稳定版而非漂移的 main）。`v0.1.0` 是修复前基线（`43b71e2`）；audit-remediation 含 breaking（#94 改名、#101 StatusLevel 合并等），值得 minor bump，故 pin `0.2.0`。**`v0.2.0` tag 在 epic→main 合并后打在 main 修复后 HEAD**（使 `from: "0.2.0"` 指向修复后代码，非修复前的 v0.1.0）。原缺陷：无版本 tag，README 让下游 `branch: "main"` | `git tag -l` 曾为空；现有 `v0.1.0`，epic→main 后打 `v0.2.0` | FR-7 | #11 |
| C6b | ✅ **已修复**（GitHub #102）——README 组件数 15→24，与 `docs/README.md` Component Index 对齐。原缺陷：README 称「15 documented components」但实际 24 个组件目录 / 26 个 Preview | `README.md` | FR-6 | #11 |
...
| C10a | ✅ **已修复**（GitHub #102）——新增 MIT `LICENSE`（Copyright 2026 Evan Wang）。原缺陷：缺 LICENSE | 根目录 | FR-6 | #11 |
| C10b | 缺 CHANGELOG | 根目录 | — | 记录不修（无版本契约，CHANGELOG 无意义） |
| C10c | ✅ **已修复**（GitHub #102）——`.gitignore` 去重 `.superpowers/`（原 line 73-74 重复块删除，保留带注释的一处）。原缺陷：`.gitignore` 中 `.superpowers/` 重复两次 | `.gitignore` | FR-6 | #11 |
| C10d | ✅ **已修复**（GitHub #102）——`.agents/` 加入 `.gitignore`；`AGENTS.md` 从父 checkout 纳入版本控制，**并修正其 pre-audit 快照的 broken 内容**（`.Codex/`→`.claude/` 路径、#93 移除的 `Color.primary/secondary/tertiary`、#97 移除的 `.getSize`/D16b misinfo、`statusDangerForeground`/`Tokens/CoreGradient.swift` 改名）、逐行对齐当前 `CLAUDE.md` + 顶部加「`CLAUDE.md` 为 source of truth」banner（checkpoint 评审 Critical，经用户裁决「修 broken + banner」）；`.claude/prds`+`.claude/epics` 早已 tracked、`.claude/omsp/` 已 ignore（核对无遗漏）。**持续 follow-up**：未来 `CLAUDE.md` 更新时须同步 `AGENTS.md`。原缺陷：`.claude/`、`.agents/`、`AGENTS.md` 状态悬空（既未 ignore 也未 tracked） | `git status` | FR-6 | #11 |

## 簇 D · 一致性与可访问性

| ID | 缺陷 | 证据 | FR | Issue |
|---|---|---|---|---|
| D1a | ✅ **已修复**（GitHub #99）——三个 icon-only 按钮补 `accessibilityLabel`（Suggestions/Send/Stop，加在 Button 上）；suggestion 为 toggle 额外补 `.isSelected` trait 播报展开态。原缺陷：`BottomInputBar` 三个 icon-only 按钮零可访问性标签 | `BottomInputBar.swift:148,158,170`（全文件 accessibility 出现 0 次） | FR-7 | #8 |
| D1b | ✅ **已修复**（GitHub #99）——`UnderlinedTabItem` 补 `.accessibilityAddTraits(self.isSelected ? .isSelected : [])`，与 `SegmentedControl.swift:118`/`Sidebar.swift:162` 同形。原缺陷：`UnderlinedTabItem` 未暴露选中态 | `UnderlinedTabBar.swift:143` | FR-7 | #8 |
| D1c | ✅ **已修复**（GitHub #99）——三者按语义区分处理：`LabelIcon`/`ChevronRightIcon` 装饰 → `accessibilityHidden(true)`（LabelIcon 注释说明 Label 已合并语义、不承诺 opt-out）；`DangerIcon` 承载语义 → `accessibilityLabel("Alert")`（用 Alert 而非 Warning 以免撞 warning token）。原缺陷：`Form` 装饰性图标未 `accessibilityHidden`；`DangerIcon` 承载语义却无 label | `Form.swift:27,78,95` | FR-7 | #8 |
| D2 | ✅ **已修复**（GitHub #100）——D2 枚举清单（4 处硬编码中文 + 4 处已英文串 + #99 的 4 串 + `AsyncButton` 复用 Loading）纳入 `Resources/en.lproj/Localizable.strings`（+`Localizable.stringsdict` 承载 `%lld more avatars` 复数；sourceLanguage=en，11 唯一 key，`Menu` 两处复用 → 13 调用点）：`BookCover` "Untitled"、`Toast` "Tap to dismiss"、`CoreMenuButton` "Menu"、`ProgressIndicator` "Loading"、`ProgressBar` "Progress"（默认值；用户传的 label 保持 verbatim）、`Tag` "Remove tag"、`AvatarGroup` "%lld more avatars"、`BottomInputBar` "Suggestions"/"Send"/"Stop"、`Form.DangerIcon` "Alert"、`AsyncButton` "Loading"（`accessibilityValue`，复用 `Loading` key）；4 处硬编码中文改英文，各调用点走 `bundle: .module`；`Package.swift` 加 `defaultLocalization: "en"`。**用 `.strings`/`.stringsdict` 而非 `.xcstrings`**：wiring spike 实证 SwiftPM CLI（`swift build`/`swift test`，非 Xcode）不编译 String Catalog，`.xcstrings` 被逐字拷入 bundle 后运行期回落成 key（#98 的 `.xcassets` 同类坑）；`en.lproj/.strings` 是 CLI 工具链能解析的运行期格式。**#99 指针已由本任务兑现**：#99 新增的 4 处英文 a11y 串（`BottomInputBar` 的 Suggestions/Send/Stop、`Form.DangerIcon` 的 Alert）已一并纳入 catalog（原 #99 指针要求「L10n sweep 须一并纳入」）。**未纳入（AC 枚举外，留 follow-up，非「全库穷尽」）**：`SearchField`（"Search"/"Clear …"/"Toggle Inspector"）、`CommentCard`（"Comment by \\(author)" 等插值）、`ProgressBar:58` 的 `"% complete"` value、`StatusRow:97` 的 combined value——约 8 处英文 a11y 串（多含动态插值），本任务按 AC 枚举清单交付、不声称库内穷尽，建议归 #102 或后续单独处理。原缺陷：硬编码中文 UI 字符串，与别处英文不一致；全库无 String Catalog | `Toast.swift`、`CoreMenuButton.swift`、`BookCover.swift`、`BottomInputBar.swift`、`Form.swift`、新增 `en.lproj/Localizable.strings`+`.stringsdict` | FR-7 | #9 |
| D3 | ✅ **已修复**（GitHub #95）——7 处文本字号迁到 coreFont（新增 `captionMono` 覆盖 RefPill+ListRow 的等宽）；3 处 Preview / 2 处图标字号明确排除。字号归一的具体变化见 NFR 例外 5。原缺陷：7 处绕过 `CoreTypography` 直接用系统字号（**「处」按调用簇计**，非按行：`RefPill` 的 5 行同类调用计为 1 处，`BottomInputBar` 的 2 行分属两个上下文计为 2 处；按行数则为 11 行 / 6 文件） | `AvatarGroup.swift:59`、`StatusRow.swift:46`、`StateLabel.swift:50`、`CommentCard.swift:56`、`RefPill.swift:34,37,40,42,45`、`BottomInputBar.swift:302,376` | FR-3 | #4 |
| D4 | ✅ **已修复**（GitHub #101）——7 处 public let 存储属性降 internal（ProgressBar/StatusRow/StateLabel/CommentCard/EventRow/TimelineItem/AvatarGroup），@testable 白盒测试不受影响；CoreBorderlessButtonStyle.role 判定保留 public——由 PublicVisibility probe（_ = style.role，#94 A2b 契约）钉死，降级会炸 probe。原缺陷：`public let` 存储属性冻结内部布局（而 `Tag`/`SearchField`/`Sidebar*`/`ListRow`/`Badge` 都保持 private） | `ProgressBar.swift:22-24`、`StatusRow.swift:32-34`、`StateLabel.swift:39-40`、`CommentCard.swift:27-31`、`EventRow.swift:23-26`、`TimelineItem.swift:39-40`、`AvatarGroup.swift:23` 、`CoreBorderlessButtonStyle.swift:74`（#94 按 A2b 的 AC 新增，是否收回由 #10 判定） | FR-7 | #10 |
| D5 | ✅ **已修复**（GitHub #101）——StatusLevel（合并后）/ SurfaceKind 补 Equatable；ButtonRoleStyleRole 补 nonisolated + Sendable, Equatable（三个调色板属性与 resolvedColor 保 @MainActor 以访问 token Color），新增 probe compareButtonRole 守卫；CoreMenuButtonStyle 补 Sendable, Equatable（internal、非 public breaking）。原缺陷：语义枚举协议不一致（部分 `Sendable, Equatable`，部分不是） | `Banner.swift:32`、`ButtonRoleStyleRole.swift:11`、`CoreMenuButton.swift:77`、`SurfaceKind` | FR-7 | #10 |
| D6a | ✅ **已修复**（GitHub #101）——StateLabel 泛型化对齐 Badge/Tag 双层形态，init 首参加标签 init(style:...)；三个 pill 组件 init 形态统一。原缺陷：三个同类 pill 组件三种 init 形态 | `StateLabel.swift:42`、`Badge.swift:83`、`Tag.swift:80` | FR-7 | #10 |
| D6b | ✅ **已修复**（GitHub #101）——StateLabel/StatusRow 补 @ViewBuilder designated init + where Label==Text 便利 init；EventRow 既有 pill（默认 EmptyView）designated init 已满足；SidebarNavigationRow 泛型化补 @ViewBuilder leading + AnyView 便利 init。原缺陷：部分组件只收 `String`，无法插图标或富文本 | `StateLabel`、`StatusRow`、`EventRow`、`SidebarNavigationRow` | FR-7 | #10 |
| D7 | ✅ **已修复**（GitHub #101）——glass: Bool 升级为 SegmentedControlStyle 四件套（协议 + Configuration + @Entry + View.segmentedControlStyle(_:)），内置 Glass/Plain 两实现，对齐 BannerStyle 形态。原缺陷：`SegmentedControl` 的 `glass: Bool` 是布尔 hack，有真实多外观需求应升级为 style 协议 | `SegmentedControl.swift:27,33` | FR-4 | #10 |
...
| D9 | ✅ **已修复**（GitHub #102）——`git mv` 至 `Components/Banner/Banner.swift`，历史保留（rename 检测 0 内容变更），活文件无残留旧路径引用（SwiftPM 按目录自动含）。原缺陷：`Banner.swift` 直接放 `Components/` 根目录，其余组件都是 `Components/<Name>/<Name>.swift` | — | FR-7 | #11 |
...
| D12 | ✅ **已修复**（GitHub #102）——机械替换：CoreMenuButton `spacing:8→CoreSpacing.sm`、`0.94→CoreButtonMetrics.pressedScale`，TimelineItem `spacing:0→CoreSpacing.none`；判断项（值零变化）：TimelineItem dotSize 32/20 提命名常量 `TimelineDotDiameter`（泛型类型不支持 static stored，落文件级 caseless enum）、CommentCard `1` 保留裸值+hairline 注释、BottomInputBar `44` 提 `minimumHitTargetSide`（HIG 值≠metrics 48）、AvatarGroup ramp 加澄清注释（判断理由见 #102 记录）。原缺陷：硬编码数值本应引用 token | `CoreMenuButton.swift:128,138`、`TimelineItem.swift:74,99-104`、`CommentCard.swift:59`、`BottomInputBar.swift:219`、`AvatarGroup.swift:33-40,76-85` | FR-7 | #11 |
...
| D15 | ✅ **已修复**（GitHub #102）——`FillColors` 四处平台分支改 `#if canImport(UIKit)/#else/#endif`，与 `SystemBackgroundColors`/`SystemLabelColors` 写法一致。原缺陷：`FillColors` 平台分支缺 `#else`，与同层两个桥接文件写法不一致；两条件皆不成立时无 return | `FillColors.swift:16-23,30-37,44-51,58-65` | FR-7 | #11 |
| D16a | ✅ **已修复**（GitHub #101 顺带，#102 核对）——#101 泛型化重写已移除旧「只列 4 style」汇总文档，`StateLabelStyle` 6 个 case 均有 inline 语义注释，`:28` 现为 `struct Spec`，无遗漏。原缺陷：`StateLabel` 文档只列 4 个 style 而枚举实际 6 个 | `StateLabel.swift:28` vs `:14-21` | FR-7 | #11 |
...
| D18 | ✅ **已修复**（GitHub #102）——Sidebar 补 `#Preview` 覆盖 6 个 public 组件各可辨识场景；FloatingGlass / TelegramGlass 两 modifier 各补 `#Preview`（置于 `surfaceCanvas` 背板）。两 trait 编译绿，iOS Simulator `Dynamic Type 布局` 断言层 TEST SUCCEEDED。原缺陷：`Sidebar`（391 行 / 6 个 public 组件）与两个 public modifier 无 `#Preview` | `Sidebar.swift`、`FloatingGlassModifier.swift`、`TelegramGlassButtonModifier.swift` | FR-7 | #11 |
...
## 附录：C2 / C5 逐文件测试处置清单（GitHub #98）

SC-5「恒真断言归零」的可机械判定前提。**首列为文件 / 目标名**（非 `^| [A-D][0-9]` ID 形态，不进顶部计数命令），行末非 `| #N |`（不进 Issue 承载数求和）。

### C2 恒真断言五文件 + 顺带清理

| 测试文件 | 处置 | 做法 / 理由 |
|---|---|---|
| `StatusColorsTests.swift` | 已改写为真断言 | 5 个 `let _: Color`（0 `#expect`）→ **24 条** asset 名断言（共享 `assetName`，见 `TestSupport.swift`）：accent/success/attention/danger 各 5 档含 border，done 4 档（无 border）；反证：篡改 `StatusColors` 某 token 的 asset 名即红 |
| `FloatingGlassModifierTests.swift` | 已改写为真断言 | `type(of:).isEmpty` 恒真 → `FloatingGlassModifier` 的 `isInteractive` 默认值 + 透传契约断言；反证：改 init 默认值 / 透传即红 |
| `SurfaceKindTests.swift` | 断言瘦身·保留编译引用（0 test） | 删三个恒真 `.count` 断言；保留非-`@Test` 的 `static apiGuard: [SurfaceKind]`（误删任一 public case 即编译失败，拦破坏性变更）。**token 映射（`.card→surfaceCard/...`）是 private，Tests/ 内不可断言，非「间接覆盖」**；文件从 3 test 降为 0 test |
| `AvatarTests.swift` | 已删除 | `avatar.name == "Alice"` 是 memberwise init 赋值（编译器保证）；首字母提取 / 哈希背景色派生均在 `body` 内联，Tests/ 不可断言。改不出真断言故删 |
| `ProgressIndicatorTests.swift` | 已删除 | `_ = ProgressIndicator()` 为 0 `#expect`；init 空、`body` 内联无暴露状态，无可改写为真断言者 |
| `CoreDesignTests.swift`（`CoreGradientTests`） | 恒真清理 + 新增真断言 | 计划外顺带（属 C2 精神）：原 `#expect(Bool(true))` → CoreGradient 的 trait 分流断言（`String(describing:)` 内省 `AnyShapeStyle` box：默认 `ColorBox`／Blossom `LinearGradient`）；反证通过 |

### C4a / C4b 新增测试

| 测试文件 | 处置 | 做法 |
|---|---|---|
| `BlossomColorDivergenceTests.swift` | 本轮补测（C4a） | accent 的 asset 名 + Contents.json 真颜色值随 trait 分流；两 trait 都跑、断言值不同、反证通过 |
| `CoreDesignTests.swift`（`BlossomAssetTests`） | 本轮补测（C4b） | asset guard 扩展到 `violet-0…9` + `cyan-1` |

### C5 零测试目标（AC 不要求全补；大多「记录不补 + 理由」）

| 目标 | 处置 | 理由 |
|---|---|---|
| `CheckBoxToggleStyle` | 记录不补 | `makeBody` 输出 SwiftUI 私有类型，Tests/ 不可断言渲染；ViewInspector 属 Out of Scope |
| `Form` | 记录不补 | 纯视图组合，无暴露的可断言派生行为；视觉走 agent 审美（C3） |
| `CoreMenuButton` | 记录不补 | 同上，视图组合无暴露状态 |
| 四个 ButtonStyle（`SolidButtonStyle`/`LightButtonStyle`/`CoreBorderlessButtonStyle`/`CircularGlassButtonStyle`） | 记录不补 | `makeBody` 输出私有类型；取色逻辑已由 `ButtonRoleStyleRole` 收敛（B3a），单元可测性归属该类型 |
| `ButtonRoleStyleRole` | **已有测试（更正清单）** | C5 审计快照记「零测试」**有误**：`ButtonStyleDefaultTests.swift` 的 `ButtonRoleStyleRole 三态取色` suite（#96/B3a 补）已覆盖 `resolvedColor` 的 disabled>pressed 优先级 + 5 role 遍历（`Color ==` 直接比较）。本轮曾误建重复测试，发现后删除 |
| 全部 token 层（`CoreSpacing`/`CoreRadius`/`CoreTypography`/…，仅 `CoreButtonMetrics` 有测） | 部分补 / 记录不补 | 纯常量声明无行为；`CoreTypography` 的缩放行为已由 #95 `DynamicTypeLayoutTests`+`coreFont` 覆盖；`CoreGradient` 本轮已补分流测试；其余纯常量记录不补 |
| 全部 modifier | 部分补 / 记录不补 | 多数 `makeBody` 输出私有类型；`FloatingGlassModifier` 本轮已补契约测试；`SurfaceModifier` 映射 private 不可断言 |
| `StarShape` | 记录不补 | D17 已记录可能为下游预留；path 几何可测但本轮 Out of Scope |
| `ColorExtension` | 记录不补（可后续补强） | `Color(text:)` 哈希派生的稳定性可测，本轮未纳入 |
| `KeyboardHandlingTests` | 已由 #97 删除 | 随 `KeyboardHandling.swift` 整文件删除（B4c） |
