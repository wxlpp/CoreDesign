...
| B1c | `FunctionalColor` 的 primary/secondary/tertiary 组仅有 2 处显式引用：`CheckBox.swift:31`（A1 的 bug 点）与 `CoreGradient+Preview.swift:17`（`Color.secondary`，删除后会**静默**改解析到 SwiftUI 内建灰色而不报错——A1 型遮蔽的又一实例，第 3 轮评审发现）。产品代码中其 Blossom 分流零效果 | `CheckBox.swift:31`、`CoreGradient+Preview.swift:17` | FR-1 | #2 |
