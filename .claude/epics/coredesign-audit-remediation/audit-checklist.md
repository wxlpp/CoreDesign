...
| C10d | ✅ **已修复**（GitHub #102）——`.agents/` 加入 `.gitignore`；`AGENTS.md` 从父 checkout 纳入版本控制（内容为 pre-audit 快照，已在 #102 记录建议后续刷新）；`.claude/prds`+`.claude/epics` 早已 tracked、`.claude/omsp/` 已 ignore（核对无遗漏）。原缺陷：`.claude/`、`.agents/`、`AGENTS.md` 状态悬空（既未 ignore 也未 tracked） | `git status` | FR-6 | #11 |
