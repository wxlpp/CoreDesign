...
| 测试文件 | 处置 | 做法 / 理由 |
|---|---|---|
| `StatusColorsTests.swift` | 已改写为真断言 | 5 个 `let _: Color`（0 `#expect`）→ 20 条 asset 名断言（`String(describing:)` 取名）；反证：篡改 `StatusColors` 某 token 的 asset 名即红 |
...
| `ButtonRoleStyleRole` | 记录不补（可后续补强） | `resolvedColor(isEnabled:isPressed:)` 返回 `Color`，asset 名可断言——本 Issue 范围限 C2/C4，记录为后续补强项 |
