        ...
        Button(action: self.action) {
            HStack(spacing: CoreSpacing.sm) {
                ...
                Text(self.title)
                    ...
                    .lineLimit(self.titleLineLimit)   // 见下方说明
...
> **`.lineLimit(nil)` 不等于「不施加 lineLimit」**——它会显式重置从祖先继承来的值。本库的 row 会被调用方包裹，祖先可能设过 `lineLimit`。若要严格等价于原实现（三个 row 根本没写 lineLimit），把骨架里那行改为条件应用：
> ```swift
>                     .modifier(OptionalLineLimit(self.titleLineLimit))
> ```
> 或更简单地让 `titleLineLimit` 只在 Document row 传值、骨架内用 `if let`。**实施时二选一并在 progress.md 记明**。
>
