...
| `.circularGlass` 的 **5** 个调用点（AsyncButton ×1 / BottomInputBar ×3 / Previews ×1）**全都没设 `controlSize`** | B3e 的默认档是 `.regular`(32) 而非 `.large`(40)，见《判断 1》 |
...
**但「接入 controlSize 后取 `.large` = 40」是错的。** 实测五个调用点：
...
方案 C 下这 5 个调用点**一行都不用改**（这正是选它的理由）。确认没有遗漏的显式构造：

```bash
echo "本文件内: $(grep -cF 'CircularGlassButtonStyle(' Sources/CoreDesign/Components/Button/styles/CircularGlassButtonStyle.swift)  (预期 2)"
grep -rn 'CircularGlassButtonStyle(' Sources App \
  --exclude=CircularGlassButtonStyle.swift 2>/dev/null; echo "文件外 rc=$?  (预期 1，即无外部直接构造)"
```
Expected: 本文件内恰好 2（都在访问器扩展内；Step 1 会在其上方插入约 15 行，故不写死行号——**载荷是计数 2，不是位置**），文件外 `rc=1`。
...
- 八项各自的做法
- **SC-8 实测行数**与 `CoreTypography` 引用数（row body 内 11 → ?；文件级 16 → ?，另 5 处在 `SidebarSection` / `SidebarStatusFooter`，不在本任务范围）
