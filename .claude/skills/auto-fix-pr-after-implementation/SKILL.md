...
description: |
  当 superpowers 的 finishing-a-development-branch 完成后（即 GitHub 上
  已创建 PR），直接启动 fix-pr 工作流：立即开始轮询 Copilot review，待
  review 完成后进入 fix-pr 处理反馈。当 PR 刚从 superpowers worktree
  打开，且尚未收到 Copilot review 时激活。
---

# 实现完成后自动进入 fix-pr 工作流

把 superpowers 的“implementation done”状态衔接到 fix-pr 反馈闭环。

## 前置条件

只有在以下条件全部满足时才激活：
- 当前会话里刚打开了一个 PR（通过 session state 跟踪，或用 `gh pr view --json createdAt` 检查最近的创建时间）
- PR 的 head branch 与当前 worktree branch 一致
...
2. **立即开始轮询等待 Copilot review**：
   ...
   Copilot review 由 PR 打开事件自动触发，这里不再主动请求首次 review。
...
5. **决定是否继续循环**：
  ...
  - 如果是 `fixed`（已经修改并推送）：说明新的 Copilot review 会再次自动触发。回到步骤 3，继续监控下一轮
