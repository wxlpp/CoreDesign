...
#   逐条实测）：在 `7c8c07e` 上，`--minimum-access-level` 那个 flag 在本文件里出现
#   **两次**（真 flag + 本节原来那句逐字重复的说明），豁免表路径出现**三次**
#   （真赋值 + 头注释 + stderr 文案）；只钉片段的话，删掉真的那份、留着注释那份，
#   `contains` 照样满足 ⇒ 脚本与树内判据双绿（PR #314 第 2 轮终审 C-1 实测的完整失效链）。
#   ⚠️ **`98f551b` 已把 flag 那份注释副本改写掉**（就是本段）⇒ 今天 flag 全文只剩 1 次，
#   路径仍是 3 次。**别把上面那两个数读成现在时**（PR #314 第 3 轮终审 C-5）。
