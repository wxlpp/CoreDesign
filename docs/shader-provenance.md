...
- **可落地数 = 2 + 24 = 26**
- **`N_B` = 10**（由 #248 spike 按「固定成本 10–16h ÷ 边际 ≈1.5h/shader」反推）
- **26 ≥ 10 ⇒ 闸②通过，`shipswift-shaders`（#243）可启动**
...
**是按移植估的，clean-room 下应上调**。⇒ **建议 B-2 / B-3 分解时按 clean-room 重估工时。**

### 需要回改的文档

- `.claude/epics/shipswift-shaders/epic.md`：
  - AD-G 补本表结论；`N_B` 填 10；SC 的"可落地数 ≥ N_B"标记为**已满足（26）**
