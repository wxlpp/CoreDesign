...
| 档 | 判据 | 对已落地件的效力 |
|---|---|---|
| **待追溯（低指纹）** | ⚠️ **判据已改写**（第 2 轮终审 C-4）：不是「函数体是不是算法定义」，而是**复制量 + 可主张的著作权保护**——① 复制量小（个位数行）；② 其中的常数/结构是**功能性**的（换一组就不工作，不是审美选择）；③ 来源为无许可的公开发表 ⇒ **plausibly 低于独创性门槛 / de minimis** | **不阻断**。条件：ⓐ 不作任何原创声称；ⓑ 有具名承接 issue（**编号必须填**，不得写「后继 issue」）；ⓒ 下一次该文件被实质修改时必须追完 |
...
| 项 | 档 | 承接 |
|---|---|---|
| `ramp3` | 低指纹 | #249 后继 issue |
| `edgeWidth` | 低指纹 | 同上 |
| `wangHash` | **低指纹** | ⚠️ **上一版的理由「已具名出处，只缺许可裁定」不是判据表里的任何一条**，且它不能区分任何东西——q/r 级联同样"已具名出处"却判强档 ⇒ 该理由在这里不做任何工作。**按改写后的判据重判**：6 行整数运算（复制量小 ✓）、常数与移位表是**功能性**的（换一组雪崩性质就坏 ✓）、Wang 的页面无许可声明 ✓ ⇒ 低指纹成立。⚠️ 但**「逐字符一致」这个事实必须同时记着**——它不改变分档，改变的是**不得作任何原创声称**这条的严格程度 |
| Teschner 素数三元组 | **低指纹** | 三个常数，功能性（空间散列的经典取值），论文公开发表 ⇒ 同上 |
| **`coreDesignRefractiveGlass` 主体** | **强指纹** | ⚠️ **阻断 epic→main**，须在 B-4 前追完 |
...
⚠️ **这正是本表存在的理由的第二次兑现**：第 1 版因为没查 Shadertoy 的默认许可而
...
- ⚠️ **但 `N_B` = 5 的分母被本表自己作废了**（第 5 轮终审 I6）：#248 spike 的
  `:310-311` 逐字写着「边际成本是从『难件 + 逐 shader 文档』推的、**不是**从
