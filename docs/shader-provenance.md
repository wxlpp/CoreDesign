...
| 档 | 判据 | 对已落地件的效力 |
|---|---|---|
| **待追溯（低指纹 · 推定惯用法）** | 函数体是该算法的**定义**或业界唯一写法，无可辨识的个人表达（`ramp3` 三档 smoothstep、`edgeWidth` 的 `max(fwidth, ε)`）| **不阻断**。条件：ⓐ 不作任何原创声称；ⓑ 有具名承接；ⓒ 下一次该文件被实质修改时必须追完 |
| **待追溯（强指纹 · 阻断）** | 有可辨识的个人表达（级联结构、变量命名、hand-tuned 常数组）——本表自评「指纹强度不低于 `InkSmoke` 的 q/r 级联」者一律入此档 | **阻断**。追完前不得合入 `main`（合入 epic 分支可，须在 PR 记账）|
...
| `edgeWidth` | 低指纹 | 同上 |
| `wangHash` / Teschner 素数 | 低指纹（已具名出处，只缺许可裁定）| 同上 |
...
- **不触发全量回溯。** 轴 6 提高的是 **`自研实现` 声称**的门槛，而：
