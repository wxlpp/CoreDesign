...
2. **调用方会合理地想要一个「看起来完全不同、但含义相同」的版本吗？**
   ...
   **从未枚举过候选**」——两处讲的是同一份 `notes`，**以此处的如实陈述为准**。
...
> `.claude/epics/component-contract/52-step3-audit.md` 的「档 5：存疑」，该文件随 #52 PR
> 落地）——所以是「**未确认有**」，不是「**确证无**」。
...
当成「优先序改变了最终落点」的例子——在 `Tag` 身上，两条路径殊途同归。
...
`.claude/epics/component-contract/52-step3-audit.md`（该文件随 #52 PR 落地），对当时全部
50 条 `step3` 逐条覆盖、无抽样）显示登记表现有 `step3` 条目中**没有已确认合格的实例**
可充当样例——另有 4 条两可、
#52 未裁，所以是「**未确认有**」，不是「**确证无**」。**这是实测状态，不是遗漏。**

⚠️ **为什么不补一条充数**：审计里最接近的两条自身都在 `notes` 里举出了 ≥2 个替代形态却仍
登记 `step3`，是**待复核条目**，不是样例——逐条见 `docs/contract-defects.md` D-44-2 的分档表
（「只满足 (B)、(A) 不满足」档，恰 2 条）。⚠️ **本段刻意不点名**：点名会撞上上方「钉为范例
按结论逐条计」条款——公约正文点名即把被点名者的 `step3` **钉死**，抢在它们自己的新尺复核
之前锁死结论，这正是它们目前仍待复核、还没被点名钉死的原因。⇒ **承认空缺优于硬造样本**，
...
|---|---|
| 1. Apple 有原生协议吗？ | ❌ 无 |
| 2. 能举出 ≥2 个业界真实替代形态吗？ | 唯一被枚举出的候选是 **pill** —— 它由兄弟组件 `Badge` 承担，按「候选形态的作用域」三条件逐条核验**被正当排除**（`Badge` 真实存在于登记表、此处写明了组件名、`Badge` 真的承担 pill 形态）。排除后 `Tag` 自己**没有成文的**剩余候选（改判前〔511576f〕实测：`实心 chip` / `描边 chip` 在 `docs/component-registry.json`、本文件与 `Sources/CoreDesign/Components/Tag/Tag.swift` 三处命中**均为 0**；`docs/contract-defects.md` 里对这两个词的命中即本条推演记录本身，不构成剩余候选的反例；本句自身对这两个词的引用同样不计入该计数）⇒ **举不出** |
| 3. 视觉是含义的一部分吗？（能**额外**说清「换了长相就不是这东西」吗？） | ❌ **说不清**——本走查原先用同一句「chip 的长相就是它的含义」回答步骤 2 与步骤 3，步骤 3 除复述步骤 2 外无独立内容 ⇒ 循环论证，不满足步骤 2「必须**额外**说清」的门槛。且 `Tag.swift` 里唯一带含义的形状陈述是「**不**使用 `.full`——这是与 Badge 的视觉区分点之一」，**明文是关系性的** ⇒ 剥掉 `Badge` 后没有可用的非关系性理由 |
| — | ⇒ 视为答不上来，**落步骤 4** |
...
无法证伪；现改为与 A.1 / A.4 一致的「能举出 ≥2 个业界真实替代形态吗？」。
