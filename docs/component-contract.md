...
>   **落地后仍为真**。
> · **机制断言**（「实现要把那个刚性拆掉」）**完全成立，且被本轮实测证成**：被拆掉的正是
>   「骨架固定留一格」这个刚性 —— 实测 leading 闭包吐 `EmptyView` 时骨架分配 **0pt 而非
>   20pt**，即该「固定」作为**渲染事实**从来不成立（拆法是**绕过**，非改源码那一行）。
>   ⚠️ 上一版这里写「部分成立、因源码一字未改故该合取项仍为真」，是**用源码文本事实去驳一条
>   渲染事实主张**的范畴错误，已改（详见 `D-64-1` ①.3）。
>   **落点仍不变**：承重证据的合取项 1（必填公开参数）与 3（位置数三变二）独立支撑结论。
