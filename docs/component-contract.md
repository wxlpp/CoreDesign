...
| **`CoreDesignEffects`** | ⚠️ **暂定 a：轻公约**（**不是已裁定**） | 唯一论据是成本模型，而该模型**已知不足以定案**——见下方《成本模型的四条缺口》与复判闸。 |
| **`CoreDesignShaders`** | ⚠️ **暂定 a**（同上） | 附加定性理由：`.metal` 侧本就在所有 SwiftSyntax 守卫射程之外（**AD-F**），走 b 只会买到半套。 |
...
- **触发器（task 246 的交付项）**：`docs/effects-registry.json` 落地时，
  **前两个真实 Effects 件必须同时按 b 走一遍判定法并记录 notes 字符数**，
  其中**至少一件须是能走「已穷尽」支的纯装饰件**（缺口 4 点名的那一档）。
- **机器痕迹**：registry schema 加 `bSideNotesChars` 字段 + 一条断言
  「前两条必须非空」。
- **复判条件**：若实测单价 < `a_fixed / (P − 每件成本)`（`a_fixed` 取上表**区间**
  而非点估算）⇒ **必须重判**，且重判结果可以是 b。
