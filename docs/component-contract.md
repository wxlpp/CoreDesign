...
> `ChapterStatus.defaultLabel` 是 `ChapterStatusBadge` 里**单独的 internal 计算属性**，
> 与 `public init(_:label:)` 的 `label` 参数是**两个不同的声明**，因而能被分别标注。
> ⚠️ **别把「类型不同」当成前提**：`defaultLabel` 现在确实是 `LocalizedStringResource`
> 而参数是 `String?`，但那是 `#43-1` 条款三**裁决的结果**（「依二提供的默认值**必须**走
