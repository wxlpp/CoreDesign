...
3. **组件的视觉是它含义的一部分吗？**（换个长相就不是这个东西了）
   ...
   状态，而门槛字面只区分了两种——
   - `notes` 不合格、组件实质**也**不满足 ⇒ **改判 `tiebreaker`**；
   - `notes` 不合格、但组件实质**满足** ⇒ **补写 `notes`，不改判**；
   - `notes` 合格 ⇒ 通过。

   ⇒ **复核既有判例时，必须走两段**：先按 `notes` 现状机械三分（通过 / 待压测 / 直接
   改判），再对「待压测」堆读**源码 + 公开 API + 渲染语义**做实质压测。只按 `notes` 现状
   判会把整批条目一次性判死——**那不是复核，是批量改判**（`D-52-5` 的操作结论对 (A) 侧
...
> `oh-my-story` main @ `a169832`；实测那 4 条全在 **StoryUI 侧**，#53 一条都没碰）——所以是
> 「**未确认有**」，不是「**确证无**」。
...
> - **StoryUI 侧 24 条尚未复核**（移交 B / #54）⇒ 该句对它们**依然为真**。
> ⇒ 21 + 4 + 24 = 49，与 `docs/contract-defects.md` `D-44-1`「一律以 **49** 为准」吻合。
> ⚠️ **门槛已在 21 条上执行且无一合格，仍推不出「`step3` 通路应当废除」**——那是覆盖 49 条的
> 全局结论，StoryUI 侧 24 条未复核前不足以支撑（承接见 `docs/contract-defects.md` 的 #53
> 收口条目）。⚠️ 本 issue **不提出废除判定法出口的修订提案**。
> 该状态若变化，须走修订回路更新本句。
