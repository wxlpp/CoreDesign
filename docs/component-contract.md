...
> **实测状态（#52 全量审计；#53 已补跑 CoreDesign 侧；#59 已按修订后判据重判 17 条压测样本；#54 已补跑 StoryUI 侧 24 条）**：登记表现有全部 `step3` 条目中，
...
| 子形态 | 形状 | 本仓既有实例 |
|---|---|---|
| **D1 外观槽** | 公开 `@ViewBuilder` 参数，让调用方替换组件**自身某一处视觉** | `TimelineItem` 的 `node:`（`Sources/CoreDesign/Components/Timeline/Timeline.swift:64`） |
| **D2 配置枚举** | 公开枚举，穷举该组件外观的合法取值 | `StepsIndicatorStyle`（`Sources/CoreDesign/Components/Steps/Steps.swift:49`） |
...
1. **有默认画法** —— 不传该槽时组件**自己画**那一处，源码里能指出对应的默认实现
   （如 `Sources/CoreDesign/Components/Timeline/Timeline.swift:220` 的
   `private var nodeView`）。⚠️ 内容槽通常**没有**默认画法，因为组件不知道调用方要放什么。
...
「`systemImage` 是必填无默认值的公开参数」（`Sources/CoreDesign/Components/Sidebar/Sidebar.swift:221`
实测：同一个 `init` 里 `trailingSystemImage` 有 `= nil`，而 `systemImage` 没有）。而实现
...
2. ⚠️ **「分格框」是 `PinCode` 的当前形态，按「替代 = 不含组件当前的形态」不该入候选** ——
   `Sources/CoreDesign/Components/PinCode/PinCode.swift` 的实现自陈「渲染层是 `length` 个
...
硬编码为 `Sources/CoreDesign`）**只是实现层盲区，不是公约层许可**。
...
| **`CoreDesignEffects`** | ⚠️ **暂定 b**（**不是已裁定**） | 成本模型**已知不足以定案**（四条缺口），而 b 是**可逆的那一侧**——见下。 |
| **`CoreDesignShaders`** | ⚠️ **暂定 b**（同上） | 同上。⚠️ 定性上仍有一条支持 a 的理由：`.metal` 侧在所有 SwiftSyntax 守卫射程之外（**AD-F**）⇒ 走 b 只买到半套；但它不足以单独撑起裁决。 |

⚠️⚠️ **`Charts = b` 与 `Effects/Shaders = a` 的证据地位不同，必须分开读**：
- **Charts 那一半现在就裁，且不依赖成本模型**——理由是定性的（判定法步骤 2 对图表真有内容），
  且下游连锁（J-2、47→51、`inspected.count` 11→15、`ComponentTextParamGuard` 五个数）
  已全部算清并回写 `255.md`。
- **Effects / Shaders 那一半只是暂定**。本裁决自陈「成本是唯一决定性论据」，
  而第 4 轮终审查出该模型有**四条独立缺口，任意两条叠加即翻到 b**（见下）。
  一份自称唯一论据的裁决，其唯一论据不能停在这个状态。
...
1. **参照物选错，且与本文自相矛盾。** 上一版取 `ReachableTypeRegistryGuard`
   （10 173）作差集守卫的经验下限，称它是「本仓唯一真做双向差集的守卫」
   ——**实测它零个 `subtracting`**，而本文档 154 行之后的拍板一里逐字写着
   「点名过的 `ReachableTypeRegistryGuard` **一处都没有**」。**同一份文档里两句打架。**
   本仓真做这件事的最小同类是 `ComponentTextParamGuard` **28 293**，是所取"下限"的 **2.8 倍**；
   而 `244.md` 的 AC **点名的参照集**是 `AccessibilityStringLiteralGuard`（13 475）
   与 `BoolParameterScanner`（49 225）。⇒ a_fixed 是**区间**，不是点估算。
2. **拍板四（1 932 字符）被误判为共有。** 它整节的内容是「根**只许**是
   `CoreDesign + Charts`，加 Effects/Shaders **为本裁决所禁止**」——**走 b 时这一节不存在**
   （根直接扩到三个 target）。按本裁决自己的反事实口径，它是 **a 特有**。
3. **单位换算从未论证。** a_fixed 的主项是 **Swift 源码字符**，b 的单价是**中文散文字符**，
   两者以 **1:1** 相减相除。一个中文字符的写作成本约是一行 Swift 里一个 ASCII 字符的
   2–4 倍 ⇒ **一个 2–4× 的未定标度，压在一个 1.1× 量级的结论上**
   ⇒ **模型在 850 那一档没有分辨力。**
4. **决定性的那一档单价从未被测。** #59 的停止规则是**二选一**：
   「≥3 个具名候选」**或**「写明已穷尽 + 基线名单」。上一版只写了贵的那一支，
   而**便宜那一支**（候选为 0 ⇒ 来源义务为 0、三分法与作用域不触发）正是
   「纯装饰、长相即含义」这类件的活路。两个样本都因为**真有候选**而走不了第二支
   ⇒ **Effects 件最可能落的那一档，单价是未知数**。

**按修正后的输入重算**（a 特有正文 = 拍板一 862 + 默认扩展点条款 1 787 + 拍板四 1 932 = 4 581）：

| 差集守卫锚 | a_fixed | 单价 1 920 | 单价 **850**（AC 指定的实测值） |
|---|---|---|---|
| AC 下锚（13 475） | 18 056 | a 胜 2.3× | **a 胜 1.03×**（噪声内） |
| 最小真差集守卫（28 293） | 32 874 | a 胜 1.6× | **b 胜 1.43×** |
| 本裁决自称的模板 `ComponentRegistryGuard`（43 858） | 48 439 | a 胜 1.2× | **b 胜 1.91×** |

⇒ **在 AC 指定的实测单价下，六格里五格 b 胜。** 上一版「a 胜 1.25×–2.9×」不成立。

##### ⚠️ 第 5 轮：默认值从 a 翻成 b —— 三条新证据，方向一致

第 4 轮我把裁决降为「暂定 a + 复判闸」。第 5 轮终审查出**三条硬伤，全部指向同一侧**，
我逐条独立复核后确认属实：

1. **复判闸的公式量纲错误，写死为「永不触发」。**
   我写的是 `unit < a_fixed / (P − 每件成本)` —— `P` 是**件数**（38）、
   `每件成本` 是**字符数**（350）⇒ `38 − 350 = −312` ⇒ 阈值 ≈ **−58**
   ⇒「实测单价 < −58」**恒为假**。整个闸门唯一的数值判据是死的。
   正解由 `a_fixed + P·per > P·unit` 得出：**`unit < a_fixed/P + 每件成本`**，
   三个锚下分别是 **825 / 1215 / 1625**
   ⇒ **AC 实测的 850 今天就已经触发其中两档。**
2. **闸门自身的成本一进模型，最后一格 a 也翻了。** 按本裁决在缺口 2 里用来把拍板四
   判为 a 特有的**同一条反事实口径**，《复判闸》整节（1 300 字符）与它规定的实物工作
   （前两件各走一遍 b 侧判定 = 2 × 850）**只在 a 路线下存在**、必须计入 a_fixed。
   代进唯一那格 a 胜的低锚：a = 34 356 vs b = 32 300 ⇒ **b 胜**。
   ⇒ **加上闸门后六格全部 b 胜**——我在用一个「成本使 a 必输」的机制去论证「暂定 a」。
3. **从未考虑对称的「暂定 b」，而两侧的后悔成本严重不对称。**
   本表自己写着 **b 侧固定成本 ≈ 0**（扫描根多加一个目录）：
   · 暂定 a 判错 ⇒ 246 先建 ≈28 293 字符的差集守卫 + 四组 schema 字段，翻 b 后**整台作废**；
   · 暂定 b 判错 ⇒ 多写若干条 notes，翻回 a 时 **notes 可复用**（`foundation/epic.md:70`
     逐字写着「样本 notes 可被 A-6 复用，省一次返工」）。
   ⚠️ 且顺序上必然是**先付 a 的全款、再决定 a 对不对**——闸门要读 registry，
   而 registry 是 a 的产物。这是一个与**成本**之间的自举环。

⇒ **裁定：默认值取可逆的那一侧，即 b。** 这与本公约在扩展点问题上的既有立场同构
（「少给扩展点是可逆的；多给扩展点不可逆」）：**模型分辨不了时，选后悔成本低的默认值，
而不是选自己一开始倾向的那个。**

⇒ **246 现在不建差集守卫**，只做扫描根多 target 化（成本 ≈ 0）；
差集守卫改为「复判翻 a 才建」。

##### 复判闸（把「暂定」变成可终结的，而不是永久悬置）

⚠️ 「暂不裁定」会卡住 task 246（它要知道建不建差集守卫）⇒ 取**暂定 + 硬复判闸**，
且闸门**必须机器可触发**，不能是散文——否则它会和被本裁决判掉的
「⚠️ 确有需要时」一样变成不可证伪的默认答案。

⚠️ **闸门的方向随默认值一起翻了**：现在要问的是「单价高到什么程度才值得改判 a」。

- **触发器挂在 task 252**（⚠️ **不是 246**）：252 是**第一个产出登记面 Effects 类型**
  的 task（`ScanningOverlay` 正是本裁决的样本 1）。
  ⚠️ 挂 246 是**落空的**——246 落地时真实 Effects 件是 **0 个**
  （`252.md` 的 `depends_on` 含 246；task 250 的 8 个微交互按 AD-2 排除、进不了主数组）
  ⇒「前两条非空」在空数组上**真空为真 ⇒ 绿**。而 `246.md` 自己就记录过同款失效形态
  （「本 task 落地时两个新根是空目录…新守卫在 0 个文件上必绿」）——同一份 PR 在别处
  防住了它、在闸门上又踩了一次。
- **252 的 AC 增项**：前两个进登记表的 Effects 件，**如实记录 b 侧 notes 字符数**
  与它走的是哪个出口（≥3 候选支 / 已穷尽支）。
  ⚠️ **不再要求「至少一件须是纯装饰件」**——那是一条**不可判定**条款：
  「纯装饰件」无定义（公约定义的是纯装饰**层**）、「能走已穷尽支」是判定的**产出**
  而不是可选的输入（本裁决的两个样本正是"因为真有候选而走不了第二支"），
  且没有兜底 ⇒ 会变成「那就跳过闸门」，即闸门被自己的前提静默关掉
  ——正是本裁决判掉的「⚠️ 确有需要时」型条款的复刻。
- **机器痕迹（fail-closed）**：registry schema 加 `bSideNotesChars`；断言写成
  「主数组条目数 ≥ 1 时，前两条的 `bSideNotesChars` 必须 > 0；条目数 = 0 时
  **闸门状态记为『未触发』，禁止据此认定任何一侧成立**」。
- **改判条件**：若实测单价 **≥ `a_fixed/P + 每件成本`**（三个锚下 **825 / 1215 / 1625**）
  ⇒ **改判 a**，并建差集守卫。
  ⚠️ **绝对阈值写死在这里**，不要求读者代公式——上一版就是因为公式量纲错、
  而没人去代过一次数，才让闸门恒假。
- **承接（第 3 轮 I-10 批过「没指定由谁裁」，这里不再复现）**：由 **252 的 DoD** 触发，
  开 `AD-4 复判` issue，结论以**修订本节正文 + 同步 246 / 251 / 252** 的形式落地。
...
⚠️ **照录，不删**——本公约的《事后补写的效力边界》要求错误留痕而不是抹掉。

1. **「三个同型条目同落 tiebreaker」是假的**。实测 `docs/component-registry.json`：
   `FloatingGlassModifier` 与 `TelegramGlassButtonModifier` 是 `prescriptive/tiebreaker`，
   而 **`SpinningModifier` 是 `semantic` / `step2` / `needsExtensionPoint: true`**。
2. **引了一段不存在的小节**。第 1 版写「本公约『⚠️ 默认给不给扩展点』一节自陈…」，
   全文 `grep` 该字符串只有两处**引用**、**没有这个标题**（另一处是 `:67` 早就存在的
   悬空自引用，第 1 版照抄了它）。
3. **把论据用反了**。那句话的真实出处是 §2 形态 D 的裁定来源（本文件「⚠️ **裁定的动机是
   不可逆性的不对称**」段），而**那段的目的恰恰是让人不必发 public 协议**。第 1 版拿它
   论证「判定法会逼你发 public 协议」。

⚠️ **更要紧的是第 4 点：`needsExtensionPoint: true` ≠ 必须发 public 样式协议。**
§2 的优先序是 **A > B > D > C**，D（槽 / 配置枚举）是合法落点，且「⚠️ 候选形态清单 ≠
扩展点需求集」明文允许「承认差异存在、但本轮不开扩展点」（形态 C，notes 写明理由）。
**本仓实证**：#59 把 5 条翻成 `semantic/step2/true`，随后由 #60/#64/#65 **全部以配置枚举
（形态 D）兑现，一个新的 public 样式协议都没发**。
⇒ 「批量生产不可逆 API 表面」在本仓的实际兑现路径下**从未发生过**。

##### 样本走查（终审 I-2：只给落点不给走查，读者无从复核）

⚠️ **第 2 版的样本 1 走查有三处缺陷，已按第 2 轮终审 C-4 重跑**，照录于末尾。

**样本 1 · `ScanningOverlay`**（在调用方内容之上扫过一条处理中光带）

- **步骤 1「无」**：写不出「可改写为『系统控件 + 该协议的自定义 style』且不丢功能」
  这句声明——最贴近的 `ProgressViewStyle` 只承载「进度 / 忙碌」，其 `Configuration`
  不含被扫描的内容本身，而本组件的语义核心正是「**这一块内容**正在被处理」。
- **步骤 2**，走「≥3 个具名候选」分支，逐条给来源：
  ① **Apple HIG** —— 不确定态 `ProgressView`（线性 / 圆形）；
  ② **Material Design 3** —— indeterminate `LinearProgressIndicator`；
  ③ **Ant Design** —— `Skeleton`（active 态微光）；
  ④ **Fluent** —— `Shimmer` 组件；
  ⑤ 最贴近的真实产品 —— Apple 自家 Live Text / Visual Look Up 的扫掠。
- **逐条判（五个全判，不留空）**：
  - ⑤ **是本组件的当前形态本身** ⇒ 按「替代 = 不含组件当前的形态」**不入候选**。
  - ③④ 同属「用占位骨架 / 微光**替换**内容」这一形态，**由兄弟组件 `Skeleton`
    与 `SkeletonCircle` 承担**（两者都在登记表，均 `prescriptive/step3`）
    ⇒ 作用域三条件满足，**排除**。
  - ①② 是**同一形态的两个体系写法**（兄弟位、行内不确定态指示器；② 的 linear
    形态本就包含在 ① 的「线性 / 圆形」里）⇒ 按皮肤变体的**互异性**要求，
    两者**塌成一个**，不是两个。
  - 该形态（兄弟位不确定态指示器）**由已登记的 `ProgressIndicator` 承担**——
    ⚠️ 提问必须在**形态级**：「`ProgressIndicator` 承不承担『兄弟位不确定态进度
    指示器』这个形态？」答案是**它就是那个东西**（其 notes 自陈 body 全部内容是
    `ProgressView` + 文案 + tint + controlSize）⇒ 作用域三条件满足，**排除**。
- ⇒ 非皮肤且未被排除的候选 **0 个** < 2 ⇒ 举得犹豫 ⇒ 出口 3 落步骤 4 ⇒
  `prescriptive` / `tiebreaker` / `needsExtensionPoint: false`。

⚠️ **第 2 版落 `semantic/step2/true` 是错的，三处缺陷**：
1. **混用了两种读法**：排除 ③ 时用形态级（「骨架屏形态由 `Skeleton` 承担」，正确），
   核验 ①② 时却改用**角色级**（「`ProgressIndicator` 不承担『覆盖在内容之上』这件事」
   ——那是 `ScanningOverlay` **自己的位置 / 角色**）。本公约《候选形态的作用域》
   条件 ③ 逐字禁止这种提问方式，并且明写「**同一批判定里不得混用两种读法**」，
   而 #59 初稿栽的正是这一跤——**更松的那一侧又一次落在产生扩展点的分支上**。
2. **没做互异性核验**：只把每个候选与**现状**比，没有两两比。①② 是同一形态。
3. **走查不完整**：写着「三分法逐条」，实际只判了 ①②③，④⑤ 没有落点。

**样本 2 · `SampleLayerEffectModifier`**（在被修饰视图之上叠一层移动高光）

- **步骤 1「无」**：本身不是任何系统控件的样式。
- **步骤 2** 候选：① Apple `redacted(reason: .placeholder)` 的占位微光；
  ② Fluent `Shimmer`；③ Ant Design `Skeleton` active 扫光；
  ④ Meta 开源的 Shimmer 库（该形态的事实原型）。
- **逐条判**：①②③④ 与现状共享同一骨架——「在被修饰视图之上叠一层移动高光，
  足迹不变、不引入内容槽、不改变排布」（实现即一句 `overlay` + 一个随时间平移的
  渐变遮罩）；彼此只换高光本身的画法（渐变形状 / 角度 / 色带宽度），
  而渐变 / 描边 / 阴影这类绘制层是补充规则 1 逐字列举的**纯装饰层**。
  已按补充规则 3 核过「不因为遮罩是半透明就归装饰」并判**不适用**：
  四种画法都落在同一被修饰视图的边界内，足迹量级不变。
  ⚠️ ②③ 另有独立排除理由：其承担者是已登记的 `Skeleton`（形态级提问）。
- ⇒ 非皮肤且未被排除的候选 **0 个** < 2 ⇒ 举得犹豫 ⇒ 出口 3 落步骤 4 ⇒ `tiebreaker`。
...
⚠️ **理由已按终审 I-8 重写**（第 1 版写的「锚文件也得新建守卫，一分不省」按它自己的账
就是错的——两条路线**都**要一条 ~10 KB 守卫，那一项是平局）：

- **承重事实（已独立复核）**：本仓 `docs/components/` 有 38 份文件，**没有任何守卫锚定它**
  ——全仓引用它的只有 `App/Sources/ComponentData.swift`、`App/Sources/Previews.swift`、
  一句测试注释和若干散文（StoryUI 的 `ComponentRosterSourceAnchorTests` 在本仓**无对应物**）。
  ⇒ 备选方案「少一张会漂移的表」的**前提在 CoreDesign 不成立**。
- **真正的取舍**：两条路线守卫成本相当；差别在**多一份枚举**（JSON） vs **多一条
  `fileExists` 锚**（锚文件）。选 JSON 是因为它能带 `provenance` / `platform` / `kind`
  字段——**许可裁定（C-6）与平台限制（NFR-3）要用**，锚文件形态承载不了。
- ⚠️ **「本仓有 8 处双向差集先例」是第 1 版的错数**：实测 `subtracting` 共 **15 处**，
  分布在 **4 个文件**（`ComponentTextParamGuard` 7 / `ComponentRegistryGuard` 4 /
  `BoolExemptionGuard` 3 / `BoolParameterScanner` 1）；点名过的
...
的手工表必漂——G-7 记在案的失效形态逐字就是「手工清单与文档一起漏 ⇒ 两集合仍相等 ⇒
判据永绿」。实现上**不是新建一台大扫描器**，是把既有 `PublicTypeCollector` 扩到
extension 成员。

⚠️ **本拍板推翻三份下游文件已经写下的相反做法**（终审 I-7）：
`.claude/epics/shipswift-effects/251.md`、`.claude/epics/shipswift-**foundation**/246.md`、
`.claude/epics/shipswift-**foundation**/epic.md` 三处都写着
（⚠️ 第 2 版把后两个写成 `.../246.md` / `.../shipswift-effects/epic.md`，
**两处路径都错**——而这一节的全部意义就是"错误引用的更正记录"，第 2 轮终审 I-5 抓到）「**按 AD-4 的裁决**手工维护并在盲区台账留痕」——它们预设了 AD-4 会选手工。
**须回改**。
⚠️ **两个机制的边界也在此钉死**：`docs/effects-registry.json` 的射程是
**public `View` / `ViewModifier` 类型**；24 个 extension 入口点走**扩展成员扫描器**，
登记进**同一份** `effects-registry.json` 的独立 `entryPoints` 数组（不另立台账）
——一份枚举，两个采集器。

**拍板三 · FR-17 的 README 索引落点 ⇒ 按 target 分叉。**

⚠️ **第 1 版「三个新 target 一律另起小节」会把判据判红**（终审 C-1）：
`ComponentRegistryGuard.readmeIndexRows`（`:338-343`）只解析
`## 组件索引` 到 `## 生成预览图 / Generating Snapshots` **之间**的行，而反向判据
`registryEntriesAreCoveredByReadme` 断言**每个非 excluded 的 `coredesign` 条目都被该范围
内的某一行覆盖**。Charts 走 b ⇒ 4 条进登记表；索引行若落在解析范围之外 ⇒ `missing` = 4
⇒ **红**。

⇒ **裁决**：
- **Charts 的 4 行进主索引**（`## 组件索引` 之内）——它走 b，登记表判据要求它在解析范围内；
- **Effects / Shaders 的行在 `## 生成预览图 / Generating Snapshots` 之后另起小节**
  ——它们走 a，不进 `component-registry.json`，不受该判据约束。
- ⚠️ **新小节若带 `<img src="snapshots/…">` 单元格，PNG 须同批提交**——
  `readmeSnapshotsExist` 扫的是**整份 README**，不只索引节（第 2 轮终审 S-5）。
...
读**同一个值**，再被 `ComponentTextParamGuard` / `ComponentExtensionPointGuard` /
`ComponentHostAliasGuard` / `NativeProtocolPurityGuard` / `ComponentJudgeMutationTests`
消费；`BoolExemptionGuard` 还直接调 `ComponentRegistryGuard.coreDesignScan()`。
a/b 分路让这一个根变得**路线敏感**：

- 根**不扩**到 Charts ⇒ Charts 的 4 条登记进表却无对应源码 ⇒
  `registryCoversCoreDesignTypes` 的 `ghosts` = 4 ⇒ **红**；
- 根**扩到 Effects** ⇒ Effects 的 public 类型出现在 `scanned` 却无登记条目 ⇒
  `missing` ⇒ **红**，且 `ComponentTextParamGuard` 会开始看见 Effects 的裸文本参数。
...
⚠️ **「继承 FR-4」目前是**散文**，对这两个 target 零机器强制**（第 2 轮终审 I-4）：
...
判定法的产物**（`kind: semantic` 才触发该义务），而这两个 target 不走判定法，J-2 无输入。

⇒ **取而代之的默认条款**：`CoreDesignEffects` / `CoreDesignShaders` 的 public 类型
**默认不给扩展点**（相当于 §2 的形态 C）；确有需要时按 §2 的形态 **A / D1 / D2** 逐案裁，
**默认不发** public 样式协议。
⚠️ **第 2 版写的是「不得发」——那比证据强**（第 2 轮终审 I-3）：撤回记录第 4 点
证明的是 `semantic` 判定**不强制**发协议，不是「发协议应被禁止」。硬禁 A/B 两种
形态会与 §2 的优先序冲突，并给一个尚未枚举的组件类**制造新的不可逆性**
（将来真有一件值得发协议的效果，正确答案被提前堵死）。
⇒ 改为：要发须走 §2 的完整优先序，并在本裁决下补一条留痕。
⚠️ 这与本仓 #60/#64/#65 的实际做法一致（`needsExtensionPoint: true` 全部以配置枚举兑现）。

⚠️ **「确有需要时」必须有触发器，否则本条只是把问题挪了个地方**（第 2 轮终审 I-2）。
路线 b 下触发器是机械的：`kind: semantic` ⇒ J-2 义务 ⇒ 由
`ComponentRegistryGuard.registrySchemaIsValid` 的 `semantic ⇒ needsExtensionPoint`
断言 + `ComponentExtensionPointGuard` 守。路线 a 没有 `kind`、不走判定法
⇒ **没有任何触发器**，默认答案「不给」会变成永久且不可证伪的。
...
- ⚠️ **但「逐字不变」不等于「永远是这个数」**（终审 C-2）：本裁决同时定了 Charts 走 b
  ...
  **这属「增」不属「放松」**，由 task 255 随 Charts 登记**同批**执行，不与 FR-10 冲突。
  第 1 版把这两个数列进「逐字不变」清单，等于给下游一条会撞墙的指令。
- ⚠️ 作用域钉死后**必须配守卫**（task 246），否则本段的净效果是把一块地方正式命名为
  无守卫空白地带，与引入它的 epic 意图相反。
