...
下面四条不保证穷尽——不是要死磕出一条勉强凑数的路径。四条都不适用时，两个合法出口：

(a) **记入豁免基线**：写入 `bool-exemptions.json`（随 #39 落地），并写明「为什么四条都不适用」的理由；
(b) **论证这个参数本不该存在**：走删除。四条选不出往往正说明这个 Bool 从一开始就不该
进 public API，不必强行套进某一条。

⚠️ **(b) 是现实中最常见的处置**，但只要没写出来，执行者面对「四条都不适用」时就只会
卡住或硬凑，不会想到「也可以论证它不该存在、直接删掉」——因此必须显式列出，
不能指望这是不言自明的默认动作。
...
|---|---|
| 1. Apple 有原生协议吗？ | ❌ 无 |
| 2. 能举出 ≥2 个业界真实替代形态吗？ | 分格框 / 锁屏圆点 / 单条下划线 —— 三者共享同一个「每位一格」布局骨架，只是格子的画法不同 ⇒ 按步骤 2 的**皮肤变体判据，不计入 ≥2**，**举得犹豫** |
| — | ⇒ 视为答不上来，**落步骤 4** |
