...
- `BorderColors`：`separator` / `opaqueSeparator` 两族；`borderFocus` / `borderSelected` 改指 `accent`（此前各自独立的固定蓝 colorset，`light #0969da` / `dark #1f6feb`），随 accent 重定向自动继承。`borderSubtle` 取 `separator.opacity(0.28)` 而非直接等于 `opaqueSeparator`，是为了保持 `subtle(0.28) < muted(0.42) < default(1.0) < strong` 的既有强弱梯度，避免与字面顺序倒挂。
