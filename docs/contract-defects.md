...
3. **机制断言**（「判定用当前形态的刚性证明差异存在，实现要把那个刚性拆掉」）**完全成立，
   且被本轮实测证成** —— ⚠️ **本条曾写成「部分成立」并给出一个反向理由，那是范畴错误，已改**
   （PR #209 终审 C1）：

   承重证据的原文（`docs/component-registry.json` 的 `SidebarUtilityRow.notes`）逐字是
   「承载性的依据是**渲染事实**：……**共享骨架固定给 leading 分配一格 iconSize 宽度**（:129），
   删掉它整行位置数由三变二」——「承载性的依据是**渲染事实**」这半句**统辖三个合取项**，
   故第 2 项是一条**渲染事实**主张：骨架**无条件**给 leading 分配 iconSize。

   ⚠️ **该渲染事实主张已被本轮实测证伪**：leading 闭包吐 `EmptyView` 时骨架分配的是 **0pt，
   不是 20pt**（有字形 111.0 / `.textOnly` 83.0 / 差值 28.0）。骨架是**条件分配**，
   「**固定**留一格」作为渲染事实**从来就不成立** —— 只是在 #64 之前没人量过 `EmptyView` 那条腿。

   ⇒ 被拆掉的**正是**「固定留一格」这个刚性，只不过拆法是**绕过**而非改那一行源码。
   **源码文本未变 ≠ 它所主张的渲染事实为真** —— 上一版正是用源码文本事实去驳一条渲染事实主张，
   与本节 ①.1 自己诊断出的「模态过强」是**同一病型**（把一类断言当另一类用），而 `D-64-1`
   存在的全部意义就是防这个。
...
`SidebarUtilityRow` 在 `:255`，原 `:221` 的 `systemImage` 参数行位移更多 —— 它除了枚举块那 +36，还多吃了 `init` 新增的
`- Parameters:` DocC。

⚠️ **本条不写它的新行号**（PR #209 终审 C4 抓到「约 +36」是错的，那是 **struct 声明**的
位移被误套到参数行上）：本轮内**同一个数字腐了两次** —— 先写 `+36`（错）、改成实测的 `:263`
后，因为又编辑了一次 DocC 注释，它当场变成 `:264`。⇒ **裸行号在活跃分支上不可靠，位移量更
不可靠**。要定位请用符号名（`SidebarUtilityRow.init` 的 `systemImage:` 参数），或
