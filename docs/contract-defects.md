...
3. **机制断言**（「判定用当前形态的刚性证明差异存在，实现要把那个刚性拆掉」）**同样是模态
   ...
   **「那个刚性」逐字指的是合取项 1。** 读公约原文的紧邻上文：「而实现其候选 1（纯文字工具行）
   就**必须让该参数可省** ⇒ **拆掉自己成立的前提**」，下一句「实现要把**那个**刚性拆掉」正是
   这句的一般化，**承接的是 `systemImage` 必填这条刚性**。而 #64 实现了候选 1 且
...
4. **本轮的独立新发现**（不是对上面三句的评价，而是对 `docs/component-registry.json` 的
   ...
   一格 iconSize 宽度**（:129），删掉它整行位置数由三变二」——「承载性的依据是**渲染事实**」
   这半句统辖三个合取项，故第 2 项是一条**渲染事实**主张：骨架**无条件**给 leading 分配
   iconSize。
