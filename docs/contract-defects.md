...
`#299` 给 `RadarChart` / `RingChart` / `ActivityHeatmap` / `NetworkGraph` 枚举出的候选
（并排条形 / 平行坐标 / 径向条 / 线性进度条 / 分段条 / 堆叠条 / 日历月视图 /
折线时间序列 / 分层布局 / 环形布局 / 网格布局）里，有相当一部分在 Apple 平台上的
**真实承担者是 Apple 自家的 Swift Charts**（`BarMark` / `LineMark` / `RectangleMark` …）。
而 Swift Charts 不是「本设计系统的一个具名组件」、也不在登记表里 ⇒ 条件 ① **结构性地
不可能成立**，作用域条款**无法援引**。
...
（含 `#312` 那批扩展点）—— 若将来条件 ① 被扩宽，四个图表的落点**可能要重判**，
