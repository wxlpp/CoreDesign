...
| 闸 | 谁 | 何时生效 |
|---|---|---|
| **第一道（真正生效的那道）** | SwiftUI，看 `properties.hasMotion` | 六条都声明 `hasMotion == true` ⇒ **RM 打开时框架直接把整条转场换成 `.opacity`**，各类型的 `body` 根本不被调用 |
| 第二道（兜底） | 各转场层 3 的三元门控 | 只在框架**没有**替换时才轮得到 |

⚠️⚠️ **内层是否可达：不可达**（#292 收口时逐条对齐的口径）。经 `.transition(.flip)`
这条正常路径，层 3 的 `isReduced` **永远读不到 `true`**。保留内层门控的理由已写在上面
