...
⇒ `ComponentContractStructureGuard.contractTablesHaveNoSplitRows`：
全文表块逐块核管道符一致性，`main` 上已存在的那一处（`#215` 的原始报告）钉成**已知违规集**。
⚠️ **钉成集合而非「至多 N 处」**：修好一处**也会红**，逼人同步删登记，
否则台账会永远留着一条已经不成立的豁免。
变异：往 G-8 行插换行 ⇒ 🔴「新出现」；清空已知集 ⇒ 🔴「新出现 1172」（防为求绿删登记）。
