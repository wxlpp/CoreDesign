...
#### R-9 附｜G-5 / G-6 / G-7 / G-8（#43 的留痕移交项，一并进「已知判据缺口」节）

⚠️ **刻意用 `####` 而不是 `### R-…`**：它是 R-9 的附属说明，不是第 13 条独立记录。
若写成 `### R-`，Task 12 的 `awk '/^### R-/'` 会数出 **13**，而结论文本钉的是 **12**
——那就是把 SC-7「70 vs 71」的病复刻在自己的验收表里。

- **来源**：`43-report.md` §4 的 T-2（`derivedDataCandidates()` 陈旧命中，静默方向）、
  T-3（扫描器两处口径边缘，fail-loud 方向）、M2（27-slug roster 方向性盲区）、
  M1（FR-4 参数级源码扫描）。
- **处置**：**各自成一行进公约的「已知判据缺口」节**（G-5 / G-6 / G-7 / G-8），实现层移交。
