...
- **验证**：`swift test` 448 tests / 67 suites 通过（2 条 known issue 与本改动无关）；
...
`#67` 的 CoreDesign PR（`wxlpp/CoreDesign#214`）在**四轮外部评审**里，有 **7 次**新的假断言
...
4. 把一次**人工核对**写成「已跑**机器自检**」—— 本仓没有任何测试核公约那 5 段的**内容**（⚠️ 不是「没有任何测试读它」—— `ComponentContractStructureGuard` 会整篇读并断言 `**C.` 这个字面存在，落在改写位点 ① 内；但它核不了内容，而内容才是重点。全称量词是反例制造机）。
