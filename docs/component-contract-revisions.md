...
- **改动前（逐字）**：三分法表 A 行 = `| **A. 组件自带 chrome** | 文案**写在组件源码里**，调用方看不见也改不了 | LocalizedStringResource |`，表后无任何关于「缺省兜底」的条文。
- **改动后（逐字）**：表后补 `⚠️ **B 类参数的缺省兜底按 A 类处置**（#43 撞上，缺陷 #43-1）：…` 并**收窄 A 的判别特征**为「调用方不能经由公开 API 修改这份默认值本身」。
- **落点**：第 4 节（**行内补句，不新增小节**）。
- **连带改动**：无。
