...
- **连带改动**（`Tag` 改判的连带面共 **7** 处，逐条）：
  ...
  3. 皮肤变体优先序段（`Tag` 是它**唯一**的举例）—— 已追加读法说明：本条优先排除 pill 后，
     ...
     「描边 chip」在登记表与公约命中**均为 0**；⚠️ **本轮回写文本对这两个词的引用不计入该计数**）
...
1. **补强分支被材料否决**——补强要求写出一条**非关系性**的独立理由，而通读 `Tag.swift`
   全文（247 行）后，唯一带含义的形状陈述是「**不**使用 `.full`——这是与 Badge 的视觉区分
...
| 项 | 处置 | 理由 |
|---|---|---|
| `D-44-3`（`Tag` 的 `step3` 结论） | **改判**（`step3` → `tiebreaker`） | 见 R-13。补强分支被材料否决（`Tag.swift` 唯一带含义的形状陈述明文关系性），维持分支被 A.2 自身的循环论证否决 |
