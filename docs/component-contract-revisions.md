...
- **来源**：`oh-my-story#76`（`#72` 的 PR-B）终审 I3 指出，G-8 行有两句在 `#72` 落地后逐字为假。
...
- **第二句**「深度 ≥1 的 canary（**条数以 story 侧 `knownDeeperParams` 常量为准**）」——
  ...
  2. ⚠️ 随即自问「**谁接得住这句**」—— 本仓 CI **只 checkout 本仓**
     ...
     `pinBumpForcesRegistrySourceOfTruth`，名字跨形态稳定），状态去对面仓读。

⚠️ **本条的元教训**：`#72` 一路在治「登记了 ≠ 守住了」，而**预先记账是它的一个变体** ——
G-5 那次用它是**因为那句话有 story 侧的 loader 会来兑现**；
这里没有任何东西会来兑现，写下去就只是**给自己排一个将来的假话**。
⇒ **写预先记账之前先问：谁、在什么时候、靠什么把它兑现掉？答不上来就别写。**
