...
- **裁断（两层）**：
  1. **落点不对称本身可解释，不是缺陷，两者均不改判**——区别不在措辞，在**候选被排除的
     机制不同**：
     | | 候选 | 排除机制 | 落点 |
     |---|---|---|---|
  ...
  2. **`Tag` 的独立理由本身经压测后不成立，但本任务不因此翻转 `decidedBy`**——`Tag` 现有
     ...
     不过**——A.2 步骤 2 原话「chip 的长相就是它的含义」正是被明令排除的消极描述，步骤 3
     直接答「✅ 是」，全表没有任何额外说明；公约自己给出了击穿它的反事实（`Tag` 的 pill
...
- **连带改动**：`docs/contract-defects.md` 新增 `D-44-3`；`D-44-2`（Task 4 提前登记，
  「若 `Tag` 也判『不过』」的条件假设）补一句「已被 Task 5 证实」。
