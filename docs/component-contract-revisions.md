...
`#67` 的 CoreDesign PR（`wxlpp/CoreDesign#214`）在**四轮外部评审**里，有 **6 次**新的假断言
**长在修复文字本身**：改 notes 改到别的组件上（锚点在表里有三处，`replace(…,1)` 取首处）·
插入换行把 G-8 表格行劈成两行 · `#43-1` 前提连写错两次 · 把一次**人工核对**写成「机器自检」·
「全文件唯一」被一条 `grep -c` 证伪 · 「两轮终审全漏」被本分支自己的 commit 正文推翻。
