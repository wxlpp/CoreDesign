...
// 的稳定性依赖"进程级暖机之后没有别的渲染插进取样之间"（`#317` 起是**三**次渲染取第三帧）。
...
struct FilterTransitionTests {
    ...
    /// 「渲两次丢掉、第三次取用」（`#317` 起；此前是渲一次丢掉）只覆盖到"同一个视图刚出现"那一档，
    /// 覆盖不到"前面渲过一大堆别的东西"。⚠️ 「加长暖机（丢掉前**三**帧）实测更糟」这条结论
    /// 记于 `#266`；`#317` 改成丢掉前**两**帧（返回第 3 帧），两者都没有能分辨的样本量
    /// ——谁也没推翻谁，别把任一条当既定事实。
    ///
