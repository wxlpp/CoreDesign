...
struct JudgementReferenceGuard {
    ...
    func everyJudgementReferenceResolves() throws {
        ...
        // 不来自那一块。⚠️ 本轮重写后这一块**不再是零命中**——实测 9 条候选落在它的
        // 注释与失败信息里（`ScanFace` 的 `allCases` 2 条、同类型的 `of` 2 条，
        // 外加 `CLAUDE.md` / `AGENTS.md` 这类文件名 5 条）。它们**一条都到不了成员核对**：
        // 前四条的类型带继承子句 ⇒ 成员集不可知，后三条的成员名在 `excludedMemberNames` 里。
        ...
        //（`scripts/` 与 `App/` 是现实的扩面方向：`CLAUDE.md` 已把它们登记成「根本没扫」），
        ...
        for face in ScanFace.allCases where face != .other {
            ...
            扫描面 `\(face.rawValue)` 上**零**条候选引用被抽出来 —— 这一面的**枚举或抽取**
            断了（两者都是静默失效：空输入 ⇒ 零违规 ⇒ 绿）。
            ⚠️ 与下面那条按面成员核对下界的分工：这一条红 = 语料侧断了；
        ...
        """)
        // 聚合下界一并留着 —— 它有一块**按面下界盖不到的独家射程**（`#287` 第 3 轮终审 聚合门槛）：
