...
// 其类型与成员必须在测试目标里真的存在。**
//
// ⚠️ 仓库根那一层是 `#287` 第 1 轮终审补进来的，代价是实测出来的：这条纪律**写在
...
//   ⚠️ **射程实测（`#287` 第 4 轮终审 S1）——写成数不写成全称句**：这条缺口此前只在
//   `SymbolTable` 的 `knowableTypes` 那段文档注释里讲机理，没在这张清单上给过量级，
//   读者查射程查不到。今天的快照（用本判据自己的符号表与可知性计算跑出来的）：
//   · 三个 test target 里 **208** 个主声明类型，其中 **67 个**（32%）成员集不可知；
//   · 但按 `Tests` / `Guard` 结尾的判据 suite / 守卫，**129 个里不可知 0 个**
//     ——本仓惯例是把它们写成不带继承子句的裸 `struct`，这条惯例是它们留在射程内的**唯一**原因；
//   · 落到引用上：类型是「测试目标主声明」的候选引用 **489** 条，因不可知被跳过 **36** 条
//     （7.4%），其余 **453** 条真的走到了成员核对（这 453 就是 J1 聚合下界读到的那个数）。
//   · 被跳过的 36 条压在 **14 个辅助类型**上，逐条：`BoolParamHit` 7、`Node` 6、
//     `BoolScanResult` 3、`ComparisonFinder` 3、`PublicTypeCollector` 3、`ScanFace` 3、
//     `ComponentJudgeScanResult` 2、`OwnerExclusionKind` 2、`ScanResult` 2，
//     `Baseline` / `ComponentJudgeCollector` / `Entry` / `RegistryFile` / `Violation` 各 1。
//   ⚠️ **本判据自己的 `ScanFace` 就在这 14 个里**（它带 `String` / `CaseIterable` 继承子句）
//   ⇒ 本文件散文里那几处形如「该枚举点成员名」的引用**不受成员核对约束**，只受「类型存在」
//   约束。J1 按面下界那一段的注释里已按这条如实标注。
//   ⚠️ 这五个数是**今天的快照**，会随语料漂移 ⇒ 只写进注释，不做断言（要断的话
//   `> 0` 那两条按面下界已经在断，且不随语料漂移）。
//   ⚠️ 反过来的全称句同样不成立：「凡带继承子句的类型都在规则 A 之外」在**机理**上对，
//   但用它推出「几乎所有类型都逃了」是错的——今天逃掉的是 7.4% 的引用，且**一条判据
//   suite 都没逃**。射程窄的是**辅助类型**那一片。
...
// - **`.claude/**` 不在扫描面内**（`#287` 第 1 轮终审 I-2 (a)）：那是 PRD / epic / 任务的
...
// - **紧跟在 CJK 字符后面的引用永远提取不到**（`#287` 第 1 轮终审 I-2 (b)）：
...
//   故留在此处登记而不动。
// - **Markdown 围栏代码块按散文扫**（`#287` 第 1 轮终审 I-2 (c)）：```` ```swift ```` 块里的
...
//
// ## ⚠️ 评审标号的口径：`第 N 轮终审 X-n` 是一个**命名空间**，不许两轮共用一个 N
//
// 本文件的注释大量用「`#287` 第 N 轮终审 I-x / S-x」回指某一条评审意见。⚠️ 这个标号
// **只有在 N 唯一时才指得到东西**——`#287` 第 3 轮终审 I-F 实测：本文件上曾有三个标签
// （`I-1` / `I-2` / `S-2`）各自同时指向两件不同的事，因为连着两个修复 commit 都自称
...
// ⚠️ `1ff9216` 那一批标注**原写「第 2 轮终审」，本轮按上表校正为「第 1 轮终审」**
// ——依据是两个 commit 的标题（前者无轮次、后者自称第 2 轮）与本次派单自称第 3 轮；
// 三者只有这一种排法互不矛盾。历史 commit 正文不改写（base 是默认分支），在此登记。
//
...
// 不拆开就会被判成悬空。
//
// ⚠️ **「按显式仓库前缀做结构性豁免」这个替代方案已评估并否掉**（`#287` 第 1 轮终审 S-2）：
...
struct JudgementReferenceGuard {
    ...
    ///
    /// ⚠️⚠️ **上一版从这条 totality 推出「按面下界不会因为归错桶而失真」——实测证伪**
    /// （`#287` 第 3 轮终审 I-C）。totality 的射程**只到「不丢」，不到「归对」**：
    /// · **排除**「漏计 ⇒ 某面恒真」——没有 `nil` 出口，走到规则 A 的引用一条都跑不掉，
    ///   五个桶恒可加。这一半原文说对了。
    /// · **不排除**「归错桶」——桶的边界由下面 `of(_:)` 的**路径前缀**定，前缀与真实目录
    ///   结构漂开时，那一面只是被**部分**掏空、残余的几条照样让 `> 0` 满足。实测变异
    ///   `OTHER-theft`（把 `Sources/` 前缀收窄成 `Sources/CoreDesign/`，模拟「某个 target
    ///   搬出 `Sources/`」这类完全现实的重构）⇒ `Sources/**` 从 98 掉到 2、96 条（98%）
    ///   流进 `other`，而**六个测试全绿**。
    /// ⇒ 归错桶这一侧由 J1 里那条独立的「`other` 桶必须为 0」兜住，不是 totality 给的。
    ...
    ///
    /// ⚠️ 根那一层是 `#287` 第 1 轮终审补进来的：`CLAUDE.md` / `AGENTS.md` 就是写着
    ...
    func everyJudgementReferenceResolves() throws {
        ...
        for guide in Self.pinnedRootDocuments {
            ...
            （`#287` 第 1 轮终审 I-1：那一层当时活着两条真悬空引用，正是从这个缺口漏出去的）。
            """)
        }
        ...
        //   被整个掐掉而全套件不响——与本 PR 的 I-3 **同族但不同形态**（`#287` 第 3 轮终审 S-b
        //   把上一版「这正是 I-3 刚修掉的形态、复发了一次」收了一格）：I-3 是**恒真断言**
        //   （断言在，只是保持它绿的机制不是它点名的那个）；F2 这一档是**压根没有断言**。
        //   共同点只有一条——绿不来自「被守住」。
        ...
        // ⚠️ **本文件的散文也在 `Tests/**` 面上**：本文件注释与字符串字面量里的引用同样
        // 被算进这几个数，所以它们会随本文件一起微动 ⇒ 更不该拿它们当断言。
        // （⚠️ 上一版这里的主语写成「**这段注释本身**」——`#287` 第 3 轮终审 S-a 把当时那
        // 一整块注释单独跑了一遍候选模式匹配，**零命中**：微动来自本文件别处的散文，
        // 不来自那一块。⚠️ 重写后这一块**不再是零命中**。
        // ⚠️ **数字按本判据自己的抽取器重测**（`#287` 第 4 轮终审 F4）：上一版这里写
        // 「实测 9 条（`allCases` 2、`of` 2、文件名 5）…前四条…后三条」，① 内部不自洽
        //（4 + 3 = 7 ≠ 9）、② 与判据口径不符——那次「实测」跑的是纯文本 pattern match，
        // 把落在**代码**里（这一块里 `ScanFace` 的 `allCases` 与 `of` 的**实际调用**）
        // 的两条也算了进去，而 `references(inSwift:)` **有意只收注释与字符串字面量**。
        // 用判据自己的抽取器跑本块（从上面那条「下界按面切分」的登记起，到 `other` 桶
        // 那条下界的失败信息止），今天恰好 8 条候选：`ScanFace` 的 `allCases` 1 条、
        // 同类型的 `of` 2 条、`CLAUDE.md` / `AGENTS.md` 这类文件名 5 条。
        // 它们**一条都到不了成员核对**：前三条的类型带继承子句 ⇒ 成员集不可知
        //（`ScanFace` 正在文件头那张清单列出的 14 个不可知辅助类型里），
        // 后五条的成员名在 `excludedMemberNames` 里。
        // ⇒ 这几个数仍不随这一块变，但「零命中」这句话已经不再成立，
        // 主语只能是**本文件的散文**。）
        // ⚠️ 每面取 `> 0` 而**不是**这五个数的快照：这些数会随语料
        // 增长而漂移，钉死只会制造无谓的红；而要检出的形态恰恰是「某一面整个不可达」，
        // `> 0` 正好卡在那条线上，且不随语料漂移。
        //
        // ⚠️ **面清单走 `ScanFace.allCases`，不是一份手写的字面量**（`#287` 第 3 轮终审 I-B）：
        // 上一版写死成 `[.sources, .tests, .docs, .rootDocuments]`，而紧邻的
        // `pinnedRootDocuments` 那段注释刚否掉的正是这个形态（「一张表而不是两处各写一份，
        // 两处会各自漂移」）——同一个文件里现造了第二份事实。将来加第 5 个面
        //（`scripts/` 与 `App/` 是现实的扩面方向：**本文件头的《找到但堵不住的路径》一节**
        //  已把它们登记成「根本没扫」——`CLAUDE.md` 只转指到那一节，自己不登记），
        // 枚举加了 case 而手写清单不会跟上 ⇒ 新面直接变成第二个 `other`。
        // 配合下面那条「`other` 必须为 0」，两条合起来对 `allCases` **完备且自动跟随枚举扩张**。
        //
        // ⚠️ **两条下界，诊断方向不同**（`#287` 第 3 轮终审 I-D）：先钉「这一面抽出过候选
        // 引用」，再钉「其中有引用走到了成员核对」。上一版只有后者，且失败信息写成全称句
        //「要么没被枚举、要么没被抽取、要么没被核对，三者都是静默失效」——**三条都不成立**
        // 的反例当场可造，见下面根面那段登记。
        let candidatesByFace = references.reduce(into: [ScanFace: Int]()) {
            $0[ScanFace.of($1.file), default: 0] += 1
        }
        let candidateBreakdown = ScanFace.allCases
            .map { "\($0.rawValue)=\(candidatesByFace[$0] ?? 0)" }
            .joined(separator: "、")
        // ⚠️ **根面这条下界的射程与脆性，如实登记**（`#287` 第 3 轮终审 I-D）：今天满足它的
        // 成员核对**恰好 8 条**，逐条枚举 —— `AGENTS.md` 的 62 / 63 / 64 / 66 行与
        // `CLAUDE.md` 的 60 / 61 / 62 / 64 行，即两份文件里**同一段**《多 target 扫描根》
        // 表格的两份拷贝，只涉三个符号（`GuardScanRoots` 的 `allRoots`、`newTargetRoots`、
        // `targetNames`）。⇒ 整个根面的成员核对承重压在这一段散文上。
        // ⚠️ 它因此**同时是个假红发生器**：把那 8 处按本文件《写作纪律》那节自己要求的形态
        // 改写（类型名与成员名各自加反引号、中间不用点连）——一次零缺陷、且**遵守本仓纪律**
        // 的纯散文编辑 —— 实测当场把成员核对那条判红（`<仓库根>/*.md=0`，另三面不变）。
        // ⚠️ 而同一版下**候选引用数**那条仍绿（根面今天 74 条候选 vs 8 条成员核对，余量约
        // 9 倍）⇒ 两条的组合把「枚举 / 抽取断了」与「这一面已经没有可核对的引用了」分开：
        // 只有后者红时，先去看这一面的散文是不是刚被改写过，而不是去查扫描器。
        for face in ScanFace.allCases where face != .other {
            try #require((candidatesByFace[face] ?? 0) > 0, """
            扫描面 `\(face.rawValue)` 上**零**条候选引用被抽出来 —— 本判据当下想到的成因
            有三种，其中两种是**失效**：① 这一面的**枚举**断了、② 这一面的**抽取**断了
            （两者都是静默失效：空输入 ⇒ 零违规 ⇒ 绿）；
            第三种**不是**失效：③ 这一面是**刚加进 `ScanFace`** 的新面，而语料侧还没跟上
            （实测：给 `ScanFace` 加第 5 个 case 表示 `scripts/**`、暂不把 `scripts/` 并进
            扫描面 ⇒ 本条当场红，而枚举没断、抽取也没断）。
            ⚠️ 三种是**本判据当下想到的**，不是穷举。
            ⚠️ 与下面那条按面成员核对下界的分工：①② 红 = 语料侧断了；
            只有下面那条红 = 语料还在、只是没有一条落进规则 A 的射程。
            各面候选引用数：\(candidateBreakdown)
            各面成员核对数：\(analysis.faceBreakdown)
            """)
            try #require(analysis.memberChecked(on: face) > 0, """
            扫描面 `\(face.rawValue)` 上抽出了 \(candidatesByFace[face] ?? 0) 条候选引用，
            却**零**条走到规则 A 的成员核对 —— 本判据能想到的成因有四种：
            ① 枚举断了、② 抽取断了、③ 成员核对分支被短路、④ 候选里没有一条落进射程
            （类型不是**测试目标**里的主声明，或它的成员集不可知）。
            ①② 已被上面那条候选引用数下界排除；③ 是**静默失效**，要查扫描器；
            ④ **不是**失效 —— 这一面的散文正好不再提任何判据符号了，
            典型触发是把提及处按《写作纪律》拆开写（根面只有 8 条成员核对，一次散文改写
            就能清零，见上面那段登记）。⚠️ 四种是**本判据当下想到的**，不是穷举。
            各面候选引用数：\(candidateBreakdown)
            各面成员核对数：\(analysis.faceBreakdown)
            """)
        }
        // ⚠️ **`other` 桶必须恒为 0**（`#287` 第 3 轮终审 I-A）：`ScanFace` 的 totality 只保证
        // 「不丢」、不保证「归对」（见该枚举的文档注释）。归错桶时那一面只被**部分**掏空、
        // 残余的几条让上面的 `> 0` 照绿 —— 实测变异 `OTHER-theft`（把 `Sources/` 前缀收窄成
        // `Sources/CoreDesign/`，模拟「某个 target 搬出 `Sources/`」）⇒ `Sources/**` 98 → 2、
        // 96 条流进 `other`，上一版**六个测试全绿**。`other` 今天是 0，但在这一条之前
        // **没有任何一条断言钉住它是 0** —— 它是个无人断言的静默汇。
        // ⚠️ **对称的 `candidatesByFace[.other] == 0` 有意不断，登记理由**（`#287` 第 4 轮终审 F7）：
        // `ScanFace.of(_:)` 在 `analyse` 里的**唯一**用途是给按面成员核对计数，它不参与
        // 「谁被核对 / 谁算违规」的任何决策 ⇒ 候选侧归错桶只能**致盲下界**（把某一面的候选
        // 数记少、极端情况下把这一面的 `> 0` 判红），永远不会让一条真悬空引用逃掉。
        // ⇒ 再加一条对称断言只会重复下面这条的诊断，故只登记不加断言。
        try #require(analysis.memberChecked(on: .other) == 0, """
        落进「扫描面之外」桶的有 \(analysis.memberChecked(on: .other)) 条 —— 说明某一面的路径口径
        与 `ScanFace.of(_:)` 漂开了，那一面的按面下界正在被残余引用假绿满足。
        各面当下值：\(analysis.faceBreakdown)
        """)
        // 聚合下界一并留着 —— 它有一块**按面下界盖不到的独家射程**（`#287` 第 3 轮终审在
        // 按面下界落地后追问「那这条还留着干嘛」，此处作答；那条意见没有标号，
        // 不写成 `I-x` / `S-x` 的形状以免读者去查一个不存在的标号——`#287` 第 4 轮终审 F5）：
        // 按面 `> 0` 只保证「每一面至少还剩 1 条」，拦不住「**四面都还非空、整体却被侵蚀到
        // 门槛以下**」——例如 `knowableTypes` 的定点迭代出现回归 ⇒ 绝大多数类型变成不可知、
        // 每面各剩零星几条，聚合从 453 掉到十几，而四条按面下界**全绿**。
        // ⚠️ 它盖不到的是反过来那一档：上面变异 G 的 201 就是从这条下面走过去的
        //（`Tests/` 面独活 202、另三面全灭）。⇒ 两组下界互补，都留着。
        try #require(analysis.memberCheckedCount > 200, """
        全部扫描面合计只有 \(analysis.memberCheckedCount) 条引用走到了规则 A 的成员核对。
        ⚠️ 各面都还有值时，这更像**成员核对的射程整体收窄**（例如可知性计算回归），
        而不是某一面断掉 —— 某一面断掉会先由上面的按面下界报出来。
        无论是哪一种，J1 在这个状态下的绿都来自「没核」而不是「核过了都对」。
        各面当下值：\(analysis.faceBreakdown)
        """)
    ...
    func scanFaceMatchesTheManifest() throws {
        ...
        // ⚠️ **「整层 ⊆ 扫描面」要两条断言组合才成立**（`#287` 第 4 轮终审 F1）：
        // `rootDocFilesEnumeratesTheWholeLayer` 钉的是 `rootDocFiles(in:)` **这个函数**
        // 枚举整层，钉不住「这个函数的产出真的进了文档扫描面」——实测变异
        // `CALLSITE-whitelist`（把同一张白名单从函数体挪到调用点 `docScanFace()`）⇒
        // 那条 fixture 与上面的超集检查**全绿**（19/19），而往仓库根放的一份含两条真
        // 悬空引用的未跟踪 `.md` 真的逃出了扫描面（同一个文件在不变异的对照组下让 J1
        // 报出**恰好 2 条** `unknownMember`，证明它本该被抓）。
        // ⇒ 这里补上缺的那一半：整层的每一份都必须出现在 `docScanFace()` 的产出里。
        let facePaths = Set(Self.docScanFace().map(\.path))
        let escaped = GuardScanRoots.rootDocFiles().filter { !facePaths.contains($0.path) }
        #expect(escaped.isEmpty, """
        仓库根这一层有 \(escaped.count) 份 `.md` 被 `rootDocFiles()` 枚举到、却**不在**
        `docScanFace()` 的产出里：\(escaped.map(\.lastPathComponent).sorted())
        —— 说明扫描面在**调用点**上有过滤（白名单 / 排除 / 去重），逃出去的那几份不受
        本判据约束，而 `pinnedRootDocuments` 那条超集检查与
        `rootDocFilesEnumeratesTheWholeLayer` 都对这种形态无感（前者只查具名四份、
        后者只查函数本身）。
        """)
    }
    ...
    /// fixture 的两个模块名。
    ///
    /// ⚠️ **它们必须以 `Tests` / `Guard` 结尾**（`#287` 第 1 轮终审 I-3）：J3-b ⑨ 断言
    ...
    func silentFixtures() {
        ...
        //   ⚠️ **这条是纵深防御，不是唯一把关的闸**（`#287` 第 1 轮终审 I-3b 实测）：
        ...
        //   这条断言就是恒真的（`#287` 第 1 轮终审 I-3 实测：初版在变异 E 下全绿）。
