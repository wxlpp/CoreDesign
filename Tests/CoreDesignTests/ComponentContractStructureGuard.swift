...
struct ComponentContractStructureGuard {
    ...
    /// **26 个**承重 `###`/`####`/`#####` 小节——tiebreaker、优先级固定、边界条款、终局条款、
    ...
    /// 其下**十一条**承重 `#####`（裁决表 / 两侧成本 / 四条缺口 / 撤回记录 / 样本走查 /
    /// 四件拍板 / 拍板四 / 第 5 轮翻 b / 复判闸 / 默认扩展点条款 / 不放松清单）
    /// 是第 16–26 条。
    /// ⚠️ **括号里必须逐条列全**——第 5 轮终审 I1 抓到上一版写「十条」却只列了八个，
    /// 而漏的恰是当轮新增的两条；本文件开头 :37-42 逐字警告过这个病型，第三次复发。
    /// ⚠️ 上一版只钉了其中四条，**漏掉的恰是裁决表本身**（第 2 轮终审 C-5）。
    ...
    static let requiredSubsections = [
        ...
        // ⚠️ **只钉 h4 标题挡不住掏空**（#244 终审 I-9）：把 AD-4 正文整段删掉、
        // 只留 `####` 标题，`swift test` 照绿——正是本文件开头自陈失守过两次的那种病。
        // 下面四条 h5 是 AD-4 里**真正被下游 task 绑住**的部分：
        // `246`（守卫形态 + 默认扩展点条款）、`251`（拍板二推翻其手工维护写法）、
        // `255`（拍板三的 README 分叉 + 47→51）、`249`（a 侧边际成本口径）、
        // task 007 / shaders B-4（README 落点）。删掉任一条都会让对应 task 失去依据。
        // ⚠️ C-5：上一版只钉了 4 条 h5，而**裁决表本身那一节没钉**——删掉它、
        // 留下其余六节，`swift test` 照绿，而 246 的条件项、255 的 Charts 登记、
        // 251 的扫描器全部失去依据。《撤回记录》同理：本公约《事后补写的效力边界》
        // 要求错误「照录，不删」，不钉住它就能被静默删掉、恢复出一份干净的初稿。
        "##### 裁决：按 target 分别定，**不一刀切**",
        "##### 两侧成本：差量口径 + 盈亏平衡（本裁决的**唯一**决定性论据）",
        "##### 撤回记录：第 1 版的三处事实错误",
        "##### 样本走查（终审 I-2：只给落点不给走查，读者无从复核）",
        "##### 一并拍板的四件下游事项",
        // ⚠️ 第 3 轮终审 C1：拍板四原是上一节下的粗体段落 ⇒ 删掉它 22 行、
        // 22 条字面量一条不缺 ⇒ 照绿。而它正是「最容易被实现层反噬的一处」。
        "##### 拍板四 · `ComponentRegistryGuard` 扫描根的作用域",
        // ⚠️ 第 4 轮终审 I-6：只钉标题挡不住"删掉整节"——实测删掉拍板二整节，
        // 23 条字面量一条不缺 ⇒ 照绿，而 251.md / 246.md 刚把依据指向它。
        // 盈亏平衡表与那两条自陈同理，是最可能被后人悄悄软化的两段。
        "##### ⚠️ 成本模型的四条缺口（第 4 轮终审，我逐条独立复核后确认属实）",
        "##### ⚠️ 第 5 轮：默认值从 a 翻成 b —— 三条新证据，方向一致",
        "##### 复判闸（把「暂定」变成可终结的，而不是永久悬置）",
        "##### 轻公约的默认扩展点条款（终审 I-6：不继承 J-2 之后无人回答扩展点问题）",
        "##### 本裁决**不**放松的东西",
    ]
