...
struct ComponentContractStructureGuard {
    ...
    /// **27 个**承重 `###`/`####`/`#####` 小节——tiebreaker、优先级固定、边界条款、终局条款、
    ...
    /// 其下**七条**（结论 + 立场说明 + 撤回记录 + 四条下游连锁）是第 15–21 条。
    /// ⚠️ **第 6 轮把 AD-4 从 11 条 h5 收到 7 条**——被删的那些钉的是**已撤回的论证**
    /// （成本模型 / 轻公约 / 复判闸 / 拍板一）。其中一条字面量是
    /// `"…（本裁决的**唯一**决定性论据）"`，而第 5 轮之后成本模型**已不是**决定性论据
    /// ⇒ 谁去改这个已知为假的标题，`swift test` 立刻红——**判据在保护一个假命题**，
    /// 正是本裁决自己批判过的那件事。
